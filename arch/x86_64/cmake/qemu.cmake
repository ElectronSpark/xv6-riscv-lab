...
# KVM acceleration: checked at runtime via KVM=1 environment variable.
# A wrapper script is generated so the decision is made at `make qemu`
# time, not at cmake-configure time.
#   KVM=1 make qemu  → uses -enable-kvm -cpu host (via sudo)
#   make qemu        → uses -cpu qemu64,+pcid (TCG)
set(QEMU_WRAPPER "${CMAKE_BINARY_DIR}/qemu-wrapper.sh")
file(WRITE "${QEMU_WRAPPER}"
"#!/bin/sh\n"
"if [ \"$KVM\" = 1 ]; then\n"
"    echo '[qemu-wrapper] KVM enabled'\n"
"    exec sudo ${QEMU_X86_EXECUTABLE} -machine pc -enable-kvm -cpu host \"$@\"\n"
"else\n"
"    exec ${QEMU_X86_EXECUTABLE} -machine pc -cpu qemu64,+pcid \"$@\"\n"
"fi\n"
)
file(CHMOD "${QEMU_WRAPPER}" PERMISSIONS OWNER_READ OWNER_WRITE OWNER_EXECUTE
...
set(X86_DISK_OPTS
    ...
    -drive file=${CMAKE_BINARY_DIR}/fs.img,if=none,format=raw,id=x0,aio=native,cache.direct=on,snapshot=on -device virtio-blk-pci,drive=x0,num-queues=${CPUS},iothread=iot0
    -drive file=${CMAKE_BINARY_DIR}/xv6fs_test.img,if=none,format=raw,id=x1,aio=native,cache.direct=on,snapshot=on -device virtio-blk-pci,drive=x1,num-queues=${CPUS},iothread=iot1
)
...
add_custom_target(qemu
    COMMAND ${QEMU_WRAPPER} -smp ${CPUS} -m ${MEMORY} -nographic -chardev stdio,id=char0,mux=on,signal=off -mon chardev=char0,mode=readline -serial chardev:char0 -serial file:diag.log -debugcon file:debugcon.log -kernel ${X86_FULL_IMAGE} ${X86_DISK_OPTS} ${X86_NET_OPTS} ${X86_CMDLINE}
    COMMAND stty sane
...
add_custom_target(qemu-gui
    COMMAND ${QEMU_WRAPPER} -smp ${CPUS} -m ${MEMORY} ${X86_VGA_OPTS} -display gtk -usb -device usb-tablet -chardev stdio,id=char0,mux=on,signal=off -mon chardev=char0,mode=readline -serial chardev:char0 -serial file:diag.log -debugcon file:debugcon.log -kernel ${X86_FULL_IMAGE} ${X86_DISK_OPTS} ${X86_NET_OPTS} ${X86_CMDLINE}
    COMMAND stty sane
    DEPENDS kernel_all fs_img
    WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
...
add_custom_target(qemu-vnc
    COMMAND ${QEMU_WRAPPER} -smp ${CPUS} -m ${MEMORY} ${X86_VGA_OPTS} -display vnc=:0 -usb -device usb-tablet -chardev stdio,id=char0,mux=on,signal=off -mon chardev=char0,mode=readline -serial chardev:char0 -serial file:diag.log -debugcon file:debugcon.log -kernel ${X86_FULL_IMAGE} ${X86_DISK_OPTS} ${X86_NET_OPTS} ${X86_CMDLINE}
    COMMAND stty sane
    DEPENDS kernel_all fs_img
    WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
...
add_custom_target(qemu-serial
    COMMAND ${QEMU_WRAPPER} -smp ${CPUS} -m ${MEMORY} -nographic -chardev stdio,id=char0,mux=on,signal=off -mon chardev=char0,mode=readline -serial chardev:char0 -serial file:diag.log -kernel ${X86_FULL_IMAGE} ${X86_DISK_OPTS} ${X86_NET_OPTS} ${X86_CMDLINE}
    COMMAND stty sane
    DEPENDS kernel_all fs_img
    WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
...
add_custom_target(qemu-debugcon
    COMMAND ${QEMU_WRAPPER} -smp ${CPUS} -m ${MEMORY} -nographic -chardev stdio,id=char0,mux=on,signal=off -mon chardev=char0,mode=readline -serial none -debugcon chardev:char0 -kernel ${X86_FULL_IMAGE} ${X86_DISK_OPTS} ${X86_NET_OPTS} ${X86_CMDLINE}
    COMMAND stty sane
    DEPENDS kernel_all fs_img
    WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
...
add_custom_target(qemu-gdb
    COMMAND ${QEMU_WRAPPER} -smp ${CPUS} -m ${MEMORY} -nographic -chardev stdio,id=char0,mux=on,signal=off -mon chardev=char0,mode=readline -serial chardev:char0 -serial file:diag.log -debugcon file:debugcon.log -kernel ${X86_FULL_IMAGE} ${X86_DISK_OPTS} ${X86_NET_OPTS} ${X86_CMDLINE} -S -gdb tcp::1234
    COMMAND stty sane
