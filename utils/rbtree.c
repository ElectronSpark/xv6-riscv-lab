...
static struct rb_node **__rb_node_link(struct rb_root *root, struct rb_node *node,
                                        struct rb_node **ret_parent) {
  struct rb_node *parent = rb_parent(node);
  if (ret_parent) *ret_parent = parent;
  if (parent == NULL) return &root->node;
  if (parent == node) return NULL;  /* Empty node */
  if (node == parent->left) return &parent->left;
  return &parent->right;
}
...
/* Left rotation */
static struct rb_node *__rb_rotate_left(struct rb_root *root, struct rb_node *node) {
...
/* Right rotation */
static struct rb_node *__rb_rotate_right(struct rb_root *root, struct rb_node *node) {
  if (node == NULL) return NULL;
  struct rb_node **link = __rb_node_link(root, node, NULL);
  struct rb_node *parent = rb_parent(node);
...
static struct rb_node *rb_insert_node(struct rb_root *root, struct rb_node *new_node) {
  if (!rb_root_is_initialized(root) || new_node == NULL) return NULL;
...
static void __rb_delete_color_fixup(struct rb_root *root, struct rb_node *node) {
...
static void __rb_do_delete_node_color(struct rb_root *root, struct rb_node **link) {
...
struct rb_node *rb_first_node(struct rb_root *root) {
  if (root == NULL || root->node == NULL) return NULL;
  struct rb_node *pos = root->node;
...
struct rb_node *rb_next_node(struct rb_node *node) {
  if (rb_node_is_empty(node)) return NULL;
...
struct rb_node *rb_find_key_rup(struct rb_root *root, uint64_t key) {
  if (!rb_root_is_initialized(root)) return NULL;
  struct rb_node *parent = NULL;
  struct rb_node **link = __rb_find_key_link(root, &parent, key);
  ...
  if (rb_keys_cmp(root, pkey, key) >= 0) return parent;
