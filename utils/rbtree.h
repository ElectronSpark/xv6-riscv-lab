...
/* Initialize a root */
static inline struct rb_root *rb_root_init(struct rb_root *root, struct rb_root_opts *opts) {
  if (root == NULL || opts == NULL) return NULL;
  if (opts->keys_cmp_fun == NULL || opts->get_key_fun == NULL) return NULL;
  root->node = NULL;
  root->opts = opts;
  return root;
}
...
static inline void rb_node_dye_as(struct rb_node *target, struct rb_node *source) {
