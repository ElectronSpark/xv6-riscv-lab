...
static uint64_t extent_get_key(struct rb_node *node) {
    struct free_extent *ext =
        (struct free_extent *)((char *)node -
                               offsetof(struct free_extent, rb_node));
    return ext->start;
}
...
static inline struct free_extent *rb_entry_extent(struct rb_node *node) {
    if (!node)
        return NULL;
    return (struct free_extent *)((char *)node -
                                  offsetof(struct free_extent, rb_node));
}
...
static struct free_extent *extent_alloc(void) {
    ...
    if (ext)
        rb_node_init(&ext->rb_node);
    return ext;
}

/* Free an extent node */
static void extent_free(struct free_extent *ext) { free(ext); }

/* Find extent with start <= blockno (floor search) */
static struct free_extent *bcache_find_extent_le(struct block_cache *bc,
                                                 uint32_t blockno) {
    return rb_entry_extent(rb_find_key_rdown(&bc->tree, blockno));
}

/* Find extent with start >= blockno (ceiling search) */
static struct free_extent *bcache_find_extent_ge(struct block_cache *bc,
                                                 uint32_t blockno) {
    return rb_entry_extent(rb_find_key_rup(&bc->tree, blockno));
}

/* Find extent containing blockno */
static struct free_extent *bcache_find_extent_containing(struct block_cache *bc,
                                                         uint32_t blockno) {
...
static void bcache_insert_extent(struct block_cache *bc, uint32_t start,
                                 uint32_t length) {
...
static uint32_t bcache_alloc_from_extent(struct block_cache *bc,
                                         struct free_extent *ext) {
...
static void bcache_mark_free(struct block_cache *bc, uint32_t blockno) {
...
static int bcache_find_free_block(struct block_cache *bc,
...
static int bcache_find_free_block_near(struct block_cache *bc, uint32_t hint,
                                       uint32_t *blockno_out) {
    if (!bc->initialized || bc->free_count == 0 || !bc->tree.node) {
        return -ENOSPC;
    }
...
static int bcache_init(struct block_cache *bc, uint32_t nblocks,
                       uint32_t data_start) {
    if (bc->initialized) {
        return 0;
    }
    ...
    bc->initialized = true;
    fprintf(stderr,
            "[xv6fs] block cache initialized: %u data blocks, %u free in %u "
            "extents\n",
...
static void bcache_destroy(struct block_cache *bc) {
...
STATIC_INLINE uint16_t from_le16(uint16_t value) {
...
STATIC_INLINE uint32_t from_le32(uint32_t value) {
#if __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__
    return value;
#else
...
STATIC_INLINE uint16_t to_le16(uint16_t value) {
#if __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__
    return value;
#else
    return __builtin_bswap16(value);
...
STATIC_INLINE uint32_t to_le32(uint32_t value) {
#if __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__
    return value;
#else
    return __builtin_bswap32(value);
...
static inline void *block_ptr(uint32_t blockno) {
...
static int sync_range(void *addr, size_t length) {
...
static int sync_range_sync(void *addr, size_t length) {
    if (!addr || length == 0 || g_fs.image == NULL) {
        return 0;
    }
...
static int sync_block(void *addr) { return sync_range(addr, BSIZE); }

static inline uint32_t bitmap_block_count(void) {
    return (g_fs.sb.nblocks + BPB - 1) / BPB;
}

static inline uint32_t data_start_block(void) {
...
static int bitmap_update(uint32_t blockno, bool set) {
    uint32_t bblock = BBLOCK(blockno, g_fs.sb);
    uint8_t *bits = block_ptr(bblock);
    if (!bits) {
        return -EIO;
...
static int alloc_block(uint32_t *blockno_out) {
    ...
    if (g_bcache.initialized) {
        ...
        /* Block is now allocated (already removed from cache by
         * find_free_block) */
 ...
 */
static int alloc_block_near(uint32_t hint, uint32_t *blockno_out) {
    if (hint == 0) {
        return alloc_block(blockno_out);
    }
 ...
 * Blocks may not be strictly consecutive but will be allocated with locality
 * hints.
 *
...
static inline struct dinode *get_inode(uint32_t inum) {
...
static inline uint16_t inode_type(const struct dinode *ip) {
    return from_le16(ip->type);
}

static inline uint16_t inode_nlink(const struct dinode *ip) {
    return from_le16(ip->nlink);
}

static inline uint32_t inode_size(const struct dinode *ip) {
    return from_le32(ip->size);
}

static int msync_inode_block(uint32_t inum);

static int inode_block_address(const struct dinode *ip, uint32_t block_index,
                               uint32_t *addr_out) {
 ...
 */
static uint32_t get_allocation_hint(const struct dinode *ip,
...
static int inode_ensure_block(uint32_t inum, struct dinode *ip,
                              uint32_t block_index, uint32_t *addr_out) {
    if (block_index < NDIRECT) {
        uint32_t addr = from_le32(ip->addrs[block_index]);
        if (addr == 0) {
    ...
    if (block_index < NINDIRECT) {
        ...
        if (addr == 0) {
            ...
            uint32_t hint = (block_index > 0)
                                ? from_le32(entries[block_index - 1])
                                : indirect;
            int rc = alloc_block_near(hint, &addr);
            if (rc != 0) {
                return rc;
...
static int inode_clear_block(uint32_t inum, struct dinode *ip,
                             uint32_t block_index) {
    if (block_index < NDIRECT) {
        uint32_t addr = from_le32(ip->addrs[block_index]);
        if (addr == 0) {
...
static ssize_t inode_read(const struct dinode *ip, void *dst, off_t offset,
                          size_t length) {
    if (offset < 0) {
        return -EINVAL;
    }
...
static ssize_t inode_write(uint32_t inum, struct dinode *ip, const char *src,
...
static int inode_truncate(uint32_t inum, struct dinode *ip, off_t length) {
    if (length < 0) {
        return -EINVAL;
    }
...
static int reset_inode(uint32_t inum, struct dinode *ip) {
...
static int read_symlink_target(uint32_t inum, char *buf, size_t bufsize,
                               size_t *len_out) {
...
static int lookup_dir_entry(uint32_t dir_inum, const char *name,
    ...
    for (uint32_t off = 0; off + sizeof(entry) <= dir_size;
         off += sizeof(entry)) {
...
static int resolve_path_internal(const char *path, uint32_t *inum_out,
                                 bool follow_final) {
    if (!path || path[0] == '\0') {
        return -ENOENT;
    }
    ...
    while (1) {
        ...
        size_t comp_len = 0;
        while (component_start[comp_len] != '\0' &&
               component_start[comp_len] != '/') {
            comp_len++;
        }
        ...
        if (type == XV6_T_SYMLINK && (!last_component || follow_final)) {
            ...
            size_t target_len = 0;
            rc = read_symlink_target(child_inum, target, sizeof(target),
                                     &target_len);
            if (rc != 0) {
                return rc;
            }
            ...
            if (target[0] == '/') {
                ...
                rc =
                    snprintf(new_path, sizeof(new_path), "/%s%s", target, rest);
            } else {
                rc = snprintf(new_path, sizeof(new_path), "%s/%s%s",
                              current_path, target, rest);
            }
        ...
        if (strcmp(component, ".") == 0) {
            ...
            if (strcmp(current_path, "/") == 0) {
                int wrote = snprintf(current_path, sizeof(current_path), "/%s",
                                     component);
...
static void dirent_copy_name(const char *src, char *dst) {
    memset(dst, 0, DIRSIZ);
    size_t len = strnlen(src, DIRSIZ);
    memcpy(dst, src, len);
}

static void dirent_to_name(const struct xv6_dirent *entry,
                           char buf[DIRSIZ + 1]) {
    size_t len = strnlen(entry->name, DIRSIZ);
...
static int dir_add_entry(uint32_t dir_inum, struct dinode *dir_ip,
    ...
    for (uint32_t off = 0; off + sizeof(entry) <= dir_size;
        ...
        if (from_le16(entry.inum) == 0) {
            ...
            rc = inode_write(dir_inum, dir_ip, (const char *)&entry, off,
                             sizeof(entry));
            if (rc < 0) {
                return (int)rc;
            }
    ...
    entry.inum = to_le16(child_inum);
    dirent_copy_name(name, entry.name);
    ssize_t wrote = inode_write(dir_inum, dir_ip, (const char *)&entry,
...
static int dir_remove_entry(uint32_t dir_inum, struct dinode *dir_ip,
                            const char *name) {
    uint32_t dir_size = inode_size(dir_ip);
    struct xv6_dirent entry;
...
static int allocate_inode(uint16_t type, uint32_t *inum_out) {
...
static mode_t mode_from_type(uint16_t type) {
    switch (type) {
    case XV6_T_DIR:
        return S_IFDIR | 0755;
    case XV6_T_FILE:
...
static int xv6_create_node(const char *path, mode_t mode, dev_t rdev,
                           uint32_t *inum_out) {
    (void)rdev;
    if (g_fs.readonly) {
        return -EROFS;
...
static int xv6_create(const char *path, mode_t mode,
...
static int xv6_mknod(const char *path, mode_t mode, dev_t rdev) {
    return xv6_create_node(path, mode, rdev, NULL);
}

static int xv6_unlink(const char *path);

static int xv6_symlink(const char *target, const char *linkpath) {
    if (g_fs.readonly) {
        return -EROFS;
    }
    if (!target || !linkpath) {
    ...
    ssize_t wrote =
        inode_write(inum, dip, (const char *)&len, 0, sizeof(size_t));
...
static int xv6_utimens(const char *path, const struct timespec tv[2],
                       struct fuse_file_info *fi) {
    (void)tv;
    (void)fi;
    ...
    /* xv6 does not store timestamps, so we acknowledge the request without
     * changes. */
    return 0;
}
...
static int xv6_unlink(const char *path) {
    ...
    memset(&entry, 0, sizeof(entry));
    ssize_t write_rc = inode_write(parent_inum, parent, (const char *)&entry,
...
static int xv6_getattr(const char *path, struct stat *stbuf,
                       struct fuse_file_info *fi) {
    (void)fi;
    ...
    if (type == XV6_T_SYMLINK) {
        ...
        stbuf->st_size =
            (link_rc == 0) ? (off_t)link_len : (off_t)inode_size(dip);
    } else {
...
static int xv6_write(const char *path, const char *buf, size_t size,
                     off_t offset, struct fuse_file_info *fi) {
    if (size == 0) {
        return 0;
    }
...
static int xv6_readlink(const char *path, char *buf, size_t size) {
    uint32_t inum;
    int rc = resolve_path_nofollow(path, &inum);
    if (rc != 0) {
        return rc;
...
static int xv6_truncate_cb(const char *path, off_t length,
                           struct fuse_file_info *fi) {
    if (g_fs.readonly) {
        return -EROFS;
    }
...
static int xv6_opendir(const char *path, struct fuse_file_info *fi) {
    uint32_t inum;
    int rc = resolve_path_follow(path, &inum);
    if (rc != 0) {
        return rc;
...
static void *xv6_init(struct fuse_conn_info *conn, struct fuse_config *cfg) {
 ...
 */
static int xv6_flush(const char *path, struct fuse_file_info *fi) {
    (void)path;
    (void)fi;
 ...
 */
static int xv6_fsync(const char *path, int isdatasync,
                     struct fuse_file_info *fi) {
    (void)path;
    (void)isdatasync;
...
static void xv6_destroy(void *userdata) {
    (void)userdata;
 ...
 */
static int xv6_statfs(const char *path, struct statvfs *stbuf) {
    (void)path;
...
static const struct fuse_opt option_spec[] = {OPTION("--image=%s", image_path),
...
static int load_superblock(void) {
    ...
    if (g_fs.image_size < (size_t)g_fs.sb.size * BSIZE) {
        fprintf(stderr,
                "[xv6fs] warning: image smaller than advertised (%zu < %u "
                "blocks)\n",
    ...
    if (rc != 0) {
        fprintf(stderr,
                "[xv6fs] warning: failed to initialize block cache: %s\n",
                strerror(-rc));
...
static int map_image(const char *path) {
    ...
    if (g_fs.fd < 0 && !g_fs.readonly) {
        ...
        fprintf(
            stderr,
            "[xv6fs] warning: %s opening '%s' read-write, retrying read-only\n",
            strerror(saved), path);
        g_fs.readonly = true;
