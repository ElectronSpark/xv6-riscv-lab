...
static void kbd_irq_handler(int irq, void *data, device_t *dev)
{
    ...
    /* Track modifier keys */
    if (code == 0x2A) { kbd_state.shift_l = !is_release; goto done; }
    if (code == 0x36) { kbd_state.shift_r = !is_release; goto done; }
    if (code == 0x1D) { kbd_state.ctrl    = !is_release; goto done; }
    if (code == 0x38) { kbd_state.alt     = !is_release; goto done; }
