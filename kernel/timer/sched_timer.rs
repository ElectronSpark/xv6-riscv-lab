...
/// C-derived parameter list. `work_callback`/`timer_callback`/
/// `sched_timer_callback` (each address-taken as a fn-pointer value passed
/// to `WorkStruct::init`/`TimerCore::timer_node_init`) stay free functions
/// below this impl block.
...
impl SchedTimer {
    ...
    fn thread_sleeping(p: *mut thread) -> bool {
        ...
        let st = Self::thread_state_get(p);
...
/// Kept as a free fn: its address is taken (`Some(work_callback)`) as a
/// `WorkStruct::init` fn-pointer value in [`SchedTimer::alloc_sched_timer_work`].
...
impl SchedTimer {
    ...
    unsafe fn free_sched_timer_work(stw: *mut SchedTimerWork) {
        ...
        unsafe { Self::slab_free(stw as *mut c_void) };
    }
}
...
impl SchedTimer {
    ...
    /// direct crate-path call (`crate::timer::sched_timer::SchedTimer::sched_timer_tick`) --
    ...
    pub(crate) fn __do_timer_tick() {
        ...
        if !was_cleared {
            ...
            unsafe { TimerCore::timer_tick(Self::sched_timer_ptr(), machine::Riscv::read_time()) };
        }
    }
}
...
impl SchedTimer {
    ...
    pub(crate) unsafe fn sched_timer_done(tn: *mut timer_node) {
        ...
        // SAFETY: caller contract.
        unsafe { TimerCore::timer_remove(tn) };
    }
    ...
    pub(crate) fn sleep_ms(ms: u64) {
        ...
        Self::sched_timer_assert(!p.is_null(), line!(), c"sleep_ms", c"Current thread must not be NULL");
        ...
        let ret = unsafe { Self::sched_timer_set(&raw mut tn, ms) };
        if ret != 0 {
        ...
        unsafe { Self::sched_timer_done(&raw mut tn) };
        machine::Riscv::intr_restore(intr);
    }
    ...
    pub(crate) fn sleep_ms_interruptible(ms: u64) -> u64 {
        ...
        let p = machine::Riscv::current_thread_ptr();
        Self::sched_timer_assert(!p.is_null(), line!(), c"sleep_ms_interruptible", c"Current thread must not be NULL");
    ...
    pub(crate) fn sched_timer_init() {
        ...
        unsafe { TimerCore::timer_init(Self::sched_timer_ptr()) };
        ...
        Self::sched_timer_assert(!wq.is_null(), line!(), c"sched_timer_init", c"Failed to create scheduler timer workqueue");

        let ret = Self::slab_cache_init(
            Self::sched_timer_work_slab_ptr(),
            c"sched_timer_work_slab".as_ptr() as *mut c_char,
            core::mem::size_of::<SchedTimerWork>(),
            crate::bindings::SLAB_FLAG_EMBEDDED as u64,
        );
        Self::sched_timer_assert(ret == 0, line!(), c"sched_timer_init", c"Failed to initialize sched_timer_work_slab");
    }
    ...
    pub(crate) unsafe fn sched_timer_add_deadline(
        ...
        let stw = unsafe { Self::alloc_sched_timer_work(deadline, callback, data) };
        if stw.is_null() {
            return -(crate::bindings::ENOMEM as c_int);
        }
        // SAFETY: `stw` is live and non-null (checked above).
        let ret = unsafe { TimerCore::timer_add(Self::sched_timer_ptr(), &raw mut (*stw).tn) };
        if ret != 0 {
            // SAFETY: `timer_add` failed, so `stw` was never linked anywhere
            // and this is the sole owner.
            unsafe { Self::free_sched_timer_work(stw) };
            return ret;
        }
        0
    }
    ...
    pub(crate) unsafe fn sched_timer_add(
        ...
        // SAFETY: caller contract forwarded unchanged.
        unsafe { Self::sched_timer_add_deadline(callback, data, deadline) }
    }
}
