...
void thread_group_exit(struct thread *p, int code) {
    ...
    list_foreach_node_safe(&tg->thread_list, t, tmp, tg_entry) {
        ...
        // properly enqueued on a run queue.  A raw __thread_state_set
        // leaves the thread in RUNNING state but off all run queues,
        // preventing it from ever being scheduled to clean up stack-
        // local resources (e.g. timer_nodes used by sleep_ms).
