...
static int __q_root_keys_cmp_fun(uint64 key1, uint64 key2) {
    ...
    // First compare node->tree.key; if equal, use node address as tie-breaker.
...
// Useful when draining a wait queue in bulk before wakeup.
// Per-node queue back-pointers are updated from @from to @to.
...
int tq_wait_cb(tq_t *q, sleep_callback_t sleep_callback,
    ...
    if (tq_enqueued(&waiter)) {
        // When the thread is woken by queue wakeup APIs, the waiter has
        // already been detached. If it was resumed asynchronously (e.g. by
        // signal handling), remove it here.
...
int tq_wait(tq_t *q, spinlock_t *lock, uint64 *rdata) {
    return tq_wait_cb(q, spin_sleep_cb, spin_wake_cb, lock, rdata);
}
...
// RB-tree keyed wait queue helper.
// For duplicate keys, this comparator mode is used with rb_find_key_rup() to
// locate the first node whose key is >= the probe, then exact key is checked.
static int __q_root_keys_cmp_rdown(uint64 key1, uint64 key2) {
    tnode_t *node1 = (tnode_t *)key1;
    tnode_t *node2 = (tnode_t *)key2;
    // Compare by key first; special-case probe node matching logic below.
    if (node1->tree.key < node2->tree.key) {
        return -1;
    } else if (node1->tree.key > node2->tree.key) {
        return 1;
 ...
 * Same protocol as tq_wait_cb(), but inserts the waiter
...
int ttree_wait_cb(ttree_t *q, uint64 key, sleep_callback_t sleep_callback,
    ...
    if (tq_enqueued(&waiter)) {
        // When the thread is woken by tree wakeup APIs, the waiter has already
        // been detached. If it was resumed asynchronously (e.g. by signal
        // handling), remove it here.
...
int ttree_wait(ttree_t *q, uint64 key, spinlock_t *lock, uint64 *rdata) {
    return ttree_wait_cb(q, key, spin_sleep_cb, spin_wake_cb, lock, rdata);
}

// Wake one waiter with a matching key.
// The node is removed from the tree before scheduler_wakeup().
...
// Wake all waiters with a matching key.
...
int ttree_wakeup_all(ttree_t *q, int error_no, uint64 rdata) {
    ...
    rb_foreach_entry_safe(&q->root, pos, n, tree.entry) {
        ...
        // Remove each node before wakeup.
