...
static inline struct fifo_subqueue *__fifo_get_subqueue(struct fifo_rq *fifo_rq,
                                                        int minor_prio) {
...
static void __fifo_put_prev_task(struct rq *rq, struct sched_entity *se) {
    ...
    // Counts (sq->count, rq->task_count) are NOT changed - task was always
    // logically "on rq".
    struct fifo_rq *fifo_rq = container_of(rq, struct fifo_rq, rq);
    int idx = __fifo_minor_prio(se);
    struct fifo_subqueue *sq = __fifo_get_subqueue(fifo_rq, idx);
...
static void __fifo_set_next_task(struct rq *rq, struct sched_entity *se) {
    ...
    // Counts (sq->count, rq->task_count) are NOT changed - task is still
    // logically "on rq".
    struct fifo_rq *fifo_rq = container_of(rq, struct fifo_rq, rq);
    int idx = __fifo_minor_prio(se);
    struct fifo_subqueue *sq = __fifo_get_subqueue(fifo_rq, idx);
...
static struct rq *__fifo_select_task_rq(struct rq *prev_rq,
    ...
    if (cpumask & (1ULL << cur_cpu)) {
        ...
        // If current CPU's subqueue is empty, use it immediately (optimal
        // locality)
        if (best_count == 0) {
            return best_rq;
        }
