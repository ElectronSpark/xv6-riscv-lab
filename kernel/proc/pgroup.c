...
/*
 * Decrement the live count on a process group.
 * Called when a thread exits.
 */
void pgroup_live_dec(struct pgroup *pg) {
    if (pg == NULL)
        return;
    assert(atomic_dec(&pg->t_cnt) > 0,
           "Process group live thread count went negative");
}
 ...
 * pgroup pointer.
 *
 * Returns the pgroup pointer, or ERR_PTR(-ESRCH) if not found.
...
struct pgroup *get_pgroup(pid_t pgid) {
    struct thread *t = get_pid_thread(pgid);
    if (IS_ERR(t))
