...
static void test_two_layer_mask(void) {
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        if (actual_group == expected_group && actual_bit == expected_bit) {
            ...
            printf("  FAIL: major %d -> group %d bit %d, expected group %d bit "
                   "%d\n",
                   major, actual_group, actual_bit, expected_group,
                   expected_bit);
        }
    }

    assert(passed == num_tests, "rq_test: bitmask mapping failed %d/%d", passed,
           num_tests);
...
static void test_priority_change(void) {
    ...
    printf("  Original priority: major=%d minor=%d\n", original_major,
           MINOR_PRIORITY(original_priority));
    ...
    assert(changed_major == new_major,
           "rq_test: major priority not changed, got %d expected %d",
           changed_major, new_major);
    assert(changed_minor == 1,
           "rq_test: minor priority not changed, got %d expected 1",
           changed_minor);
...
static void test_rq_selection(void) {
    ...
    assert(rq1 == rq2 && rq2 == rq3,
           "rq_test: inconsistent rq selection: %d, %d, %d", rq1->class_id,
           rq2->class_id, rq3->class_id);
...
// - Top layer: 8-bit mask for groups (0-7), each group covers 8 major
// priorities
...
static int verify_priority(struct sched_entity *se, int expected_major,
                           int expected_minor) {
    struct sched_attr attr;
    sched_getattr(se, &attr);
    ...
    if (actual_major != expected_major || actual_minor != expected_minor) {
        printf("    FAIL: expected (%d,%d) got (%d,%d)\n", expected_major,
               expected_minor, actual_major, actual_minor);
        return 0;
    }
    return 1;
}
...
// 6. Verify processes activated in priority order (lower major = higher
// priority)
...
static void test_priority_ordered_activation(void) {
    ...
    for (int i = 0; i < PRIORITY_TEST_COUNT; i++) {
        test_procs[i] =
            kthread_create("prio_test", priority_test_proc_entry, i, 0, 0);
        assert(!IS_ERR_OR_NULL(test_procs[i]),
               "rq_test: kthread_create failed for process %d", i);
        ...
        printf("    Created process %d (pid=%d) with priority major=%d on CPU "
               "%d\n",
               i, test_procs[i]->pid, MAJOR_PRIORITY(test_priorities[i]),
               test_cpu);
    }
    ...
    assert(correct, "rq_test: Priority ordering failed! Processes did not "
...
static void test_affinity_change(void) {
    ...
    assert(new_cpu == cur_cpu,
           "rq_test: CPU changed despite affinity pin, was %d now %d", cur_cpu,
           new_cpu);
