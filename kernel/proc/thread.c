...
static void __pcb_init(struct thread *p, struct vfs_fdtable *fdtable) {
    ...
    /* kqueue: init proc event knote list */
    spin_init(&p->kqueue_proc_lock, "kqueue_proc");
    list_entry_init(&p->kqueue_proc_knotes);
    p->thread_group = NULL;
    p->fs = NULL;
    p->fdtable = fdtable;
    ...
    p->ptrace_tracer_tgid = 0;
    p->ptrace_real_parent_pid = 0;
...
// Filesystem state and the fdtable are allocated separately by the VFS.
...
struct thread *thread_create(void *entry, uint64 arg1, uint64 arg2,
    ...
    p->fdtable = vfs_fdtable_init();
    if (IS_ERR_OR_NULL(p->fdtable)) {
        page_free(kstack, kstack_order);
        return IS_ERR(p->fdtable) ? ERR_CAST(p->fdtable) : ERR_PTR(-ENOMEM);
    }
