/*
 * Early Allocator - Buddy-style memory allocator used during kernel initialization
 * 
 * DESIGN OVERVIEW:
 * ================
 ...
 * 
 * KEY FEATURES:
 * - Small objects (≤ 64KB): Allocated from buddy-style free lists, aligned to their size
 * - Large objects (> 64KB): Allocated by advancing pointer, with user-specified alignment
...
#define EARLYALLOC_ORDERS (EARLYALLOC_LARGEST_ORDER - EARLYALLOC_SMALLEST_ORDER + 1)

// Alignment utilities
#define EARLYALLOC_ALIGN_MASK(align)   ((align) - 1)
#define EARLYALLOC_ALIGN(x, align)      \
    ((typeof(x))(((uint64)(x) + EARLYALLOC_ALIGN_MASK(align)) & ~EARLYALLOC_ALIGN_MASK(align)))

/*
 * Chunk structure - represents a free memory block
 * Must be power-of-2 sized and aligned to its size for buddy system compatibility
 */
...
static void __add_chunk_to_freelist(struct earalloc_chunk *chunk) {
    ...
    if (list_idx >= 0 && list_idx < EARLYALLOC_ORDERS) {
        list_entry_push_back(&earalloc_params.free_lists[list_idx], &chunk->list_entry);
    }
}
...
static void __free_region_to_chunks(uint64 start, uint64 end) {
    while (start < end) {
        ...
        while (order >= EARLYALLOC_SMALLEST_ORDER) {
            ...
            if ((start & (chunk_size - 1)) == 0) {
                ...
                if (order <= EARLYALLOC_LARGEST_ORDER) {
                    struct earalloc_chunk *chunk = __make_chunk((void *)start, chunk_size);
        ...
        // If we couldn't create a chunk (too small or couldn't align), skip this byte
...
void early_allocator_init(void *pa_start, void *pa_end) {
    ...
    uint64 start_aligned = EARLYALLOC_ALIGN((uint64)pa_start, EARLYALLOC_SMALLEST_CHUNK);
    uint64 end = (uint64)pa_end;
    
    assert(start_aligned < end, "early_allocator_init: invalid memory range after alignment");
 ...
 * 1. First check if a chunk of exact size exists in the target order's free list
...
static struct earalloc_chunk *__get_chunk_from_freelist(int target_order) {
    ...
    if (!LIST_IS_EMPTY(&earalloc_params.free_lists[list_idx])) {
        list_node_t *entry = list_entry_pop_back(&earalloc_params.free_lists[list_idx]);
        struct earalloc_chunk *chunk = container_of(entry, struct earalloc_chunk, list_entry);
        assert(chunk->magic == EARLYALLOC_CHUNK_MAGIC, "__get_chunk_from_freelist: corrupted chunk");
        return chunk;
    }
    
    // Try to find a larger chunk and split it buddy-style
    for (int order = target_order + 1; order <= EARLYALLOC_LARGEST_ORDER; order++) {
        ...
        if (!LIST_IS_EMPTY(&earalloc_params.free_lists[idx])) {
            ...
            list_node_t *entry = list_entry_pop_back(&earalloc_params.free_lists[idx]);
            struct earalloc_chunk *chunk = container_of(entry, struct earalloc_chunk, list_entry);
            assert(chunk->magic == EARLYALLOC_CHUNK_MAGIC, "__get_chunk_from_freelist: corrupted chunk");
            ...
            while (order > target_order) {
                ...
                struct earalloc_chunk *buddy = __make_chunk((void *)((uint64)chunk + half_size), half_size);
...
static void *__alloc_by_advancing(size_t size, size_t align) {
    ...
    // Free any unused space before the aligned address as properly aligned chunks
...
void *early_alloc_align(size_t size, size_t align) {
    ...
    if (actual_size <= EARLYALLOC_LARGEST_CHUNK && order <= EARLYALLOC_LARGEST_ORDER) {
        ...
        if (chunk != NULL) {
            // Chunk is already aligned to its size (verified in __add_chunk_to_freelist)
            return (void *)chunk;
        }
    ...
    // For large objects (larger than largest chunk), allocate with user-specified alignment
...
void *early_alloc_end_ptr(void) {
    return earalloc_params.current;
}
