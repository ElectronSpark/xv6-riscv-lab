 ...
 *
 * Disables interrupts, enqueues a stack-local waiter node, runs
 * @sleep_callback, yields, runs @wakeup_callback, and self-removes
 * if resumed asynchronously. Returns waiter.error_no.
 */
...
/**
 * tq_wait - sleep on a list queue using spinlock callbacks
 * @q: queue to wait on
 * @lock: spinlock to release before sleeping
 * @rdata: optional output for data passed by the waker (may be NULL)
 *
 * Convenience wrapper around tq_wait_cb() using spin_sleep_cb/spin_wake_cb.
 */
 ...
 * @rdata: optional output for data passed by the waker (may be NULL)
 *
 * Tree equivalent of tq_wait_cb(): waiter is inserted into the RB tree
 * using @key and removed by wakeup path (or self-removed on async resume).
 */
...
/**
 * ttree_wait - sleep on a tree queue using spinlock callbacks
 * @q: tree to wait on
 * @key: sort key for the waiter
 * @lock: spinlock to release before sleeping
 * @rdata: optional output for data passed by the waker (may be NULL)
 *
 * Convenience wrapper around ttree_wait_cb() using spin_sleep_cb/spin_wake_cb.
 */
