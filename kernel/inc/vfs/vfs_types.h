 ...
 * mount:
 *   Create and fully initialize a superblock for the filesystem, returning it
 * in `ret_sb`. Implementations should allocate the superblock, fill in its
 * fields, and leave it in an unmounted state (mountpoint/parent unset) so that
 * the VFS core can attach it to the mount tree. The returned superblock should
 * have its root_inode preloaded and the root inode's ref count set to 1. free:
 ...
 *         thus write lock of the superblock is held to protect the inode hash
 * list. Note: Filesystem drivers should zero-initialize and fill in the
 * necessary fields of the returned inode, but should NOT lock or mark it valid.
 * The VFS core will lock and mark the inode valid after adding it to the
 ...
 *   hold the superblock write lock during the operation. Thus, implementations
 * do not need to acquire additional locks on the superblock structure, and if
 * wait is false, write lock should be acquired in other threads. The `wait`
 * parameter indicates whether the operation should be synchronous. Return 0 on
 ...
 * The VFS core acquires the inode mutex before invoking inode operation
 * callbacks. Operations will require write lock on the superblock:
 * - create
 * - mkdir
 * - rmdir
 * - unlink
...
struct vfs_inode_ops {
    ...
    int (*unlink)(struct vfs_dentry *dentry,
                  struct vfs_inode *target); // Remove a file or directory entry
    struct vfs_inode *(*mkdir)(struct vfs_inode *dir, mode_t mode,
                               const char *name, size_t name_len);
    int (*rmdir)(struct vfs_dentry *dentry, struct vfs_inode *target);
    struct vfs_inode *(*mknod)(
        struct vfs_inode *dir, mode_t mode, dev_t dev, const char *name,
        size_t name_len); // Create a file of special types
    int (*move)(struct vfs_inode *old_dir, struct vfs_dentry *old_dentry,
                struct vfs_inode *new_dir, const char *name,
                size_t name_len); // Move (rename) a file or directory within
                                  // the same filesystem
    ...
    void (*destroy_inode)(
        struct vfs_inode *inode); // Release on-disk inode resources
    void (*free_inode)(
        struct vfs_inode *inode); // Release in-memory inode structure
...
struct vfs_fdtable {
    ...
    uint64 files_bitmap[(NOFILE + 63) / 64];
    uint64 cloexec_bitmap[(NOFILE + 63) / 64];
    int ref_count;
};
