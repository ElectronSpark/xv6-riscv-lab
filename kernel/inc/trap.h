...
// Dirrefent from exceptions, interrupts are triggered unexpectedly and can
// occur at any time. Thus, each CPU hart has a dedicated interrupt stack to
// handle interrupts safely. When in IRQ stack, we should:
// - Finish handling the interrupt as soon as possible
// When in IRQ stack, we should NOT:
// - Call functions that may sleep or yield the CPU. IRQ context doesn't belong
// to any thread.
