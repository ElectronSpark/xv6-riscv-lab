...
static inline void virtqueue_init(struct virtqueue *vq, struct virtq_desc *desc,
                                  struct virtq_avail *avail,
                                  struct virtq_used *used, uint16 num,
...
static inline int virtqueue_alloc_desc(struct virtqueue *vq) {
    return freelist_alloc(&vq->desc_freelist);
}

// Free a single descriptor
static inline void virtqueue_free_desc(struct virtqueue *vq, int i) {
...
static inline void virtqueue_free_chain(struct virtqueue *vq, int i) {
    while (1) {
...
static inline int virtqueue_available_desc(struct virtqueue *vq) {
    return freelist_available(&vq->desc_freelist);
}

// Add a buffer to the available ring
// head is the first descriptor in the chain
static inline void virtqueue_add_buf(struct virtqueue *vq, uint16 head) {
...
static inline int virtqueue_has_used_buf(struct virtqueue *vq) {
    __atomic_thread_fence(__ATOMIC_SEQ_CST);
...
static inline int virtqueue_get_used_buf(struct virtqueue *vq, uint32 *len) {
