...
static inline void freelist_init(struct freelist *fl, char *free, uint16 *list,
                                 uint16 max_items) {
    fl->free = free;
    fl->list = list;
...
static inline int freelist_alloc(struct freelist *fl) {
    if (fl->idx <= 0) {
        return -1;
    }
...
static inline int freelist_free(struct freelist *fl, int i) {
...
static inline int freelist_available(struct freelist *fl) { return fl->idx; }

// Check if a specific item is free
static inline int freelist_is_free(struct freelist *fl, int i) {
    if (i < 0 || i >= fl->max_items)
        return 0;
    return fl->free[i];
}
