 ...
 *
 * @return int  当 key1 小于 key2
 * 时返回一个负数；相等时返回0；大于时返回一个正数。
 */
 ...
 * 二叉树节点有有父节点时返回该节点在其父节点上的链接指针，否则 返回NULL。
 */
extern struct rb_node **__rb_node_link(struct rb_root *root,
                                       struct rb_node *node,
 ...
 *
 * @return struct rb_root*
 * 初始化成功返回初始化后的二叉树根结构体，否则返回NULL。
 */
 ...
 * @param link 当前节点在其父节点中的链接。
 */
static inline void __rb_delink_node(struct rb_node **link,
                                    struct rb_node *node) {
    *link = NULL;
 ...
 */
extern void __rb_replace_node(struct rb_node **link, struct rb_node *new_node,
 ...
 */
extern void __rb_transplant(struct rb_root *root, struct rb_node *new_node,
                            struct rb_node *old_node);
 ...
 */
extern struct rb_node **__rb_find_key_link(struct rb_root *root,
                                           struct rb_node **ret_parent,
                                           unsigned long key);
 ...
 *
 * @return struct rb_node*
 * 如果找到key值对应的节点，返回指向该节点的指针，否则返回NULL。
 */
 ...
 * @return struct rb_node*
 */
extern struct rb_node *rb_insert_node(struct rb_root *root,
 ...
 *
 * @return struct rb_node*
 * 完成旋转操作后，指向位于进行旋转的二叉树节点原来位置的新节点。
 */
extern struct rb_node *__rb_rotate_left(struct rb_root *root,
                                        struct rb_node *node);
 ...
 * 完成旋转操作后，指向位于进行旋转的二叉树节点原来位置的新节点。
 */
extern struct rb_node *__rb_rotate_right(struct rb_root *root,
                                         struct rb_node *node);
 ...
 *
 * @return type*
 * 如果有下一个节点，返回下一个节点所在结构体的指针，否则返回NULL。
 */
 ...
 *
 * @return type*
 * 如果有上一个节点，返回上一个节点所在结构体的指针，否则返回NULL。
 */
...
#define rb_foreach_entry_safe(root, pos, n, member)                            \
    for ((pos) = rb_first_entry_safe(root, typeof(*(pos)), member),            \
        (n) = rb_next_entry_safe(pos, member);                                 \
         (pos) != NULL; (pos) = (n), (n) = rb_next_entry_safe(pos, member))
...
#define rb_foreach_entry_safe_inv(root, pos, n, member)                        \
    for ((pos) = rb_last_entry_safe(root, typeof(*(pos)), member),             \
        (n) = rb_prev_entry_safe(pos, member);                                 \
         (pos) != NULL; (pos) = (n), (n) = rb_prev_entry_safe(pos, member))
