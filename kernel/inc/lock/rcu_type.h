...
// Use a single pending list per CPU. Callbacks record their registration
// timestamp. A callback is ready when: callback.timestamp <= min(all other
// CPUs' rcu_timestamp) This avoids complex segment pointer management and
// ready/not-ready list separation.

// Per-CPU RCU data structure
// Aligned to cache line to prevent false sharing between CPUs
typedef struct {
    // Timestamp when this CPU last context switched (stored in
    // cpu_local->rcu_timestamp) Used for grace period detection - all CPUs must
    // have context switched after grace period start We don't store timestamp
    // here, we read it from mycpu()->rcu_timestamp
