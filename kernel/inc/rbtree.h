 ...
 * 在定义红黑树节点的结构体时，规定红黑树节点必须按 8
 * 字节对齐，所以红黑树节点的指针的最低几位
 * 中至少有一位能保证是0，红黑树的颜色就储存在此处。
 * 规定 __parent_color 元素的最低位为 1 时为黑节点，为 0
 * 时为红节点，且NULL节点颜色为黑色。
 *
 ...
 * 如果key值发生冲突，返回红黑树中相应的节点；如果key值没冲突，
 *                         返回新插入的节点；插入失败返回NULL。
 */
extern struct rb_node *rb_insert_color(struct rb_root *root,
                                       struct rb_node *node);
 ...
 */
extern struct rb_node *rb_delete_node_color(struct rb_root *root,
                                            struct rb_node *node);
 ...
 * @return struct rb_node* 删除失败返回NULL，删除成功返回被删除的节点。
 */
extern struct rb_node *rb_delete_key_color(struct rb_root *root,
                                           unsigned long key);
