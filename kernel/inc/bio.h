...
// Return true if the iterator is still valid, false if it reaches the end of the bio
static inline void bio_iter_next_seg(struct bio *bio, struct bio_iter *it)  
{
    ...
    it->blkno = bio->blkno + (bio->done_size >> (BLK_SIZE_SHIFT + bio->block_shift));
    it->bvec_idx = bvec_idx;
}

static inline bool bio_iter_copy_bvec(struct bio *bio, struct bio_iter *it, struct bio_vec *bvec)
{
...
#define bio_for_each_segment(bvec, bio, iter) \
    for (bio_iter_start(bio, iter); \
         bio_iter_copy_bvec(bio, iter, bvec); \
...
static inline int bio_dir_write(struct bio *bio)
{
    return bio->rw;
}
...
int bio_validate(struct bio *bio, blkdev_t *blkdev);  // Validate the fields of a bio against the target block device
