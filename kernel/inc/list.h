...
#include "types.h"
#include "list_type.h"
#include <smp/atomic.h> /* For memory barriers: smp_wmb, smp_rmb, smp_mb, etc. */
 ...
 */
static inline void __list_entry_add_rcu(list_node_t *new, list_node_t *prev,
                                        list_node_t *next) {
    new->next = next;
    new->prev = prev;
 ...
 * It is safe to run concurrently with RCU readers.
 */
static inline void list_entry_add_tail_rcu(list_node_t *head,
                                           list_node_t *entry) {
 ...
 * @member: the list_node_t member within the node structure
 */
#define list_node_del_rcu(node, member) list_entry_del_rcu(&((node)->member))
...
#define list_foreach_entry_rcu(head, pos)                                      \
    for ((pos) = list_next_rcu(head); !LIST_ENTRY_IS_HEAD((head), (pos));      \
...
#define list_foreach_entry_continue_rcu(head, pos)                             \
    for (; !LIST_ENTRY_IS_HEAD((head), (pos)); (pos) = list_next_rcu(pos))
...
//   - member:
//       the member of the list entry in the structure of the node
#define list_node_detach(node, member) list_entry_detach(&((node)->member))
...
#define list_foreach_entry(head, pos)                                          \
    for ((pos) = LIST_FIRST_ENTRY(head); !LIST_ENTRY_IS_HEAD((head), (pos));   \
         (pos) = LIST_NEXT_ENTRY(pos))

#define list_foreach_entry_continue(head, pos)                                 \
    for (; !LIST_ENTRY_IS_HEAD((head), (pos)); (pos) = LIST_NEXT_ENTRY(pos))
...
#define list_for_each_entry_continue_safe(head, pos, tmp)                      \
    for ((tmp) = LIST_NEXT_ENTRY(pos); !LIST_ENTRY_IS_HEAD((head), (pos));     \
         (pos) = (tmp), (tmp) = LIST_NEXT_ENTRY(tmp))
...
#define list_foreach_node_continue_safe(head, pos, tmp, member)                \
    for ((tmp) = LIST_NEXT_NODE(head, pos, member); pos != NULL;               \
         (pos) = (tmp), (tmp) = LIST_NEXT_NODE(head, pos, member))

#define list_foreach_entry_inv(head, pos)                                      \
    for ((pos) = LIST_LAST_ENTRY(head); !LIST_ENTRY_IS_HEAD((head), (pos));    \
         (pos) = LIST_PREV_ENTRY(pos))

#define list_foreach_entry_inv_continue(head, pos)                             \
    for (; !LIST_ENTRY_IS_HEAD((head), (pos)); (pos) = LIST_PREV_ENTRY(pos))
...
#define list_for_each_entry_inv_continue_safe(head, pos, tmp)                  \
    for ((tmp) = LIST_PREV_ENTRY(pos); !LIST_ENTRY_IS_HEAD((head), (pos));     \
         (pos) = (tmp), (tmp) = LIST_PREV_ENTRY(tmp))
...
#define list_foreach_node_inv_continue_safe(head, pos, tmp, member)            \
    for ((tmp) = LIST_PREV_NODE(head, pos, member); pos != NULL;               \
         (pos) = (tmp), (tmp) = LIST_PREV_NODE(head, pos, member))
...
#define list_find_next(head, last, member, ret, __match_cond)                  \
    ({                                                                         \
        ...
        if ((ret) != NULL) {                                                   \
            list_foreach_node_continue_safe(head, ret, __list_tmp_ptr,         \
                                            member) {                          \
                if (__match_cond)                                              \
                    break;                                                     \
            }                                                                  \
...
#define list_find_prev(head, last, member, ret, __match_cond)                  \
    ({                                                                         \
        ...
        if ((ret) != NULL) {                                                   \
            list_foreach_node_inv_continue_safe(head, ret, __list_tmp_ptr,     \
                                                member) {                      \
                if (__match_cond)                                              \
                    break;                                                     \
            }                                                                  \
 ...
 */
static inline void __list_entry_splice_rcu(list_node_t *list, list_node_t *prev,
                                           list_node_t *next) {
