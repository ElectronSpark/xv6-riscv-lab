...
/// RISC-V PLIC driver ZST (P3-OO mesh sweep, see `git show 91ef670` for
/// the established convention). No receiver: every fn below keeps its
/// exact C-derived parameter list.
...
impl Plic {
    ...
    fn s_priority_thresh(hart: u32) -> *mut u32 {
        (Self::plic_base() + PLIC_PRIORITY_THRESH_BASE + ((hart as u64) << 13) + 0x1000) as *mut u32
    }

    /// `PLIC_SCLAIM(hart)`: this hart's S-mode claim/complete register.
    #[inline(always)]
    fn s_claim(hart: u32) -> *mut u32 {
        (Self::plic_base() + PLIC_CLAIM_BASE + ((hart as u64) << 13) + 0x1000) as *mut u32
    }

    /// `PLIC_PRIORITY(irq)`: the priority register for PLIC source `irq`.
    #[inline(always)]
    fn priority(irq: u32) -> *mut u32 {
        (Self::plic_base() + ((irq as u64) << 2)) as *mut u32
    }
    ...
    fn s_enable_base(hart: u32) -> *mut u32 {
        (Self::plic_base() + PLIC_ENABLE_BASE + ((hart as u64) << 8) + 0x80) as *mut u32
    }

    /// Core PLIC initialization. Device-specific IRQ priorities are set by
    /// each device's init function via [`Plic::plic_enable_irq`].
    ...
    pub(crate) extern "C" fn plicinithart() {
        ...
        unsafe {
            core::ptr::write_volatile(Self::s_priority_thresh(hart), 0);
        }
    }
    ...
    pub(crate) extern "C" fn plic_claim() -> c_int {
        ...
        unsafe { core::ptr::read_volatile(Self::s_claim(hart)) as c_int }
    }
    ...
    pub(crate) extern "C" fn plic_complete(irq: c_int) {
        ...
        unsafe {
            core::ptr::write_volatile(Self::s_claim(hart), irq as u32);
        }
    }
    ...
    pub(crate) extern "C" fn plic_enable_irq(irq: c_int) {
        ...
        unsafe {
            core::ptr::write_volatile(Self::priority(irq), 1);
            for hart in 0..crate::bindings::NCPU {
                let word = Self::s_enable_base(hart).add(word_index);
