...
//
// These ARE the kernel-wide Rust definitions of `struct trapframe` and
// `struct utrapframe` (`kernel/inc/trapframe.h`) now: `build.rs`
// blocklists the bindgen-generated forms and injects `pub use
// crate::irq::trap::Trapframe as trapframe;` / `pub use crate::irq::
// trap::Utrapframe as utrapframe;`, so every `crate::bindings::
// trapframe` / `crate::bindings::utrapframe` path across the crate
// resolves here.
//
// DANGER ZONE — ASM CONTRACT: unlike every previous N-wave type, these
// two have *assembly* consumers. kernel/irq/kernelvec.S and kernel/irq/
// trampoline.S address fields through the `TRAPFRAME_*` / `UTRAPFRAME_*`
// macros of asm-offsets.h (see kernel/inc/asm-offsets.h since P3-5;
// generated by scripts/gen_asm_offsets.py from the C header before it).
// The hardcoded const asserts below therefore cite the exact .S macro
// each field's offset feeds; any layout drift fails the target build
// before an image can be linked.
//
// Layout evidence (three-way agreement): the golden gcc-generated
// asm-offsets.h TRAPFRAME_*/UTRAPFRAME_* values + the
// riscv64-unknown-elf-gcc `_Static_assert` probe (rv64gc/lp64d,
// scratchpad p3_5_static_assert_probe.c) + the pre-nativization bindgen
// output (plain `#[repr(C)]`, all-u64 fields, derives Copy/Clone).
//
// Derive fidelity: bindgen derived Copy/Clone for both; the natives keep
// them, and build.rs's `NativeTypeCallbacks` answers Copy=Yes (the only
// by-value embedder of `trapframe` was `utrapframe` itself, native in
// the same wave; every remaining use is a pointer).
// ===========================================================================

/// Native `struct trapframe` (`kernel/inc/trapframe.h`) — the register
/// save area pushed by `kernelvec.S` on a supervisor-mode trap (and
/// embedded at offset 0 of [`Utrapframe`] for user traps). C attribute
/// `__ALIGNED(8)` == the natural `#[repr(C)]` alignment of an all-`u64`
/// struct.
#[repr(C)]
#[derive(Copy, Clone)]
...
/// Native `struct utrapframe` (`kernel/inc/trapframe.h`) — the
/// per-thread trap save area used by `trampoline.S`'s `uservec`/
/// `userret` (sits in its own page just under the trampoline page; see
/// the C header's comment, preserved there).
#[repr(C)]
#[derive(Copy, Clone)]
...
// gcc-generated asm-offsets.h define named in the message (the macros
// kernelvec.S/trampoline.S load/store through), independently confirmed
// by the toolchain-gcc `_Static_assert` probe and the pre-nativization
// bindgen output. These asserts are the drift tripwire for the .S code:
// asm-offsets.h is a static checked-in file as of P3-5, and any change
// to these types that moves a field fails the target build right here.
const _: () = {
    // struct trapframe — consumed by kernelvec.S (kernel trap entry/exit)
    // and trampoline.S (via `UTRAPFRAME_TRAPFRAME + TRAPFRAME_*` == the
    // same offsets, utrapframe embeds it at 0).
    assert!(core::mem::size_of::<Trapframe>() == 184, "TRAPFRAME_SIZE (kernelvec.S stack carve)");
    assert!(core::mem::align_of::<Trapframe>() == 8, "trapframe alignment (__ALIGNED(8))");
    assert!(core::mem::offset_of!(Trapframe, ra) == 0, "TRAPFRAME_RA (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, sp) == 8, "TRAPFRAME_SP (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, s0) == 16, "TRAPFRAME_S0 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, t0) == 24, "TRAPFRAME_T0 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, t1) == 32, "TRAPFRAME_T1 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, t2) == 40, "TRAPFRAME_T2 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, a0) == 48, "TRAPFRAME_A0 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, a1) == 56, "TRAPFRAME_A1 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, a2) == 64, "TRAPFRAME_A2 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, a3) == 72, "TRAPFRAME_A3 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, a4) == 80, "TRAPFRAME_A4 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, a5) == 88, "TRAPFRAME_A5 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, a6) == 96, "TRAPFRAME_A6 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, a7) == 104, "TRAPFRAME_A7 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, t3) == 112, "TRAPFRAME_T3 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, t4) == 120, "TRAPFRAME_T4 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, t5) == 128, "TRAPFRAME_T5 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, t6) == 136, "TRAPFRAME_T6 (kernelvec.S/trampoline.S)");
    assert!(core::mem::offset_of!(Trapframe, sepc) == 144, "TRAPFRAME_SEPC (kernelvec.S)");
    assert!(core::mem::offset_of!(Trapframe, sstatus) == 152, "TRAPFRAME_SSTATUS (kernelvec.S)");
    assert!(core::mem::offset_of!(Trapframe, scause) == 160, "TRAPFRAME_SCAUSE (kernelvec.S)");
    assert!(core::mem::offset_of!(Trapframe, stval) == 168, "TRAPFRAME_STVAL (kernelvec.S)");
    assert!(core::mem::offset_of!(Trapframe, stvec) == 176, "TRAPFRAME_STVEC (kernelvec.S)");
    // struct utrapframe — consumed by trampoline.S (uservec/userret).
    assert!(core::mem::size_of::<Utrapframe>() == 344, "UTRAPFRAME_SIZE");
    assert!(core::mem::align_of::<Utrapframe>() == 8, "utrapframe alignment (__ALIGNED(8))");
    assert!(
        core::mem::offset_of!(Utrapframe, trapframe) == 0,
        "UTRAPFRAME_TRAPFRAME (trampoline.S addresses the embedded trapframe's TRAPFRAME_* at 0)"
    );
    assert!(core::mem::offset_of!(Utrapframe, s1) == 184, "UTRAPFRAME_S1 (trampoline.S)");
    assert!(core::mem::offset_of!(Utrapframe, s2) == 192, "UTRAPFRAME_S2 (trampoline.S)");
    assert!(core::mem::offset_of!(Utrapframe, s3) == 200, "UTRAPFRAME_S3 (trampoline.S)");
    assert!(core::mem::offset_of!(Utrapframe, s4) == 208, "UTRAPFRAME_S4 (trampoline.S)");
    assert!(core::mem::offset_of!(Utrapframe, s5) == 216, "UTRAPFRAME_S5 (trampoline.S)");
    assert!(core::mem::offset_of!(Utrapframe, s6) == 224, "UTRAPFRAME_S6 (trampoline.S)");
    assert!(core::mem::offset_of!(Utrapframe, s7) == 232, "UTRAPFRAME_S7 (trampoline.S)");
    assert!(core::mem::offset_of!(Utrapframe, s8) == 240, "UTRAPFRAME_S8 (trampoline.S)");
    assert!(core::mem::offset_of!(Utrapframe, s9) == 248, "UTRAPFRAME_S9 (trampoline.S)");
    assert!(core::mem::offset_of!(Utrapframe, s10) == 256, "UTRAPFRAME_S10 (trampoline.S)");
    assert!(core::mem::offset_of!(Utrapframe, s11) == 264, "UTRAPFRAME_S11 (trampoline.S)");
    assert!(core::mem::offset_of!(Utrapframe, irq_sp) == 272, "UTRAPFRAME_IRQ_SP (trampoline.S)");
    assert!(
        core::mem::offset_of!(Utrapframe, irq_entry) == 280,
        "UTRAPFRAME_IRQ_ENTRY (trampoline.S)"
    );
    assert!(
        core::mem::offset_of!(Utrapframe, kernel_satp) == 288,
        "UTRAPFRAME_KERNEL_SATP (trampoline.S)"
    );
    assert!(
        core::mem::offset_of!(Utrapframe, kernel_sp) == 296,
        "UTRAPFRAME_KERNEL_SP (trampoline.S)"
    );
    assert!(
        core::mem::offset_of!(Utrapframe, kernel_trap) == 304,
        "UTRAPFRAME_KERNEL_TRAP (trampoline.S)"
    );
    assert!(core::mem::offset_of!(Utrapframe, tp) == 312, "UTRAPFRAME_TP (trampoline.S)");
    assert!(
        core::mem::offset_of!(Utrapframe, kernel_hartid) == 320,
        "UTRAPFRAME_KERNEL_HARTID (trampoline.S)"
    );
    assert!(core::mem::offset_of!(Utrapframe, gp) == 328, "UTRAPFRAME_GP (trampoline.S)");
    assert!(
        core::mem::offset_of!(Utrapframe, kernel_gp) == 336,
        "UTRAPFRAME_KERNEL_GP (trampoline.S)"
    );
};
...
/// Trap entry/exit surface -- ZST housing every free fn this file used to
...
/// C-derived parameter list. The four `#[no_mangle]` asm entry points
/// (`user_kirq_entrance`/`usertrap`/`kerneltrap`/`kernel_irq`) and
/// `__user_kirq_return` (address-taken as a `sw_noret_cb_t` fn-pointer
/// value passed to `__switch_noreturn`) stay free functions below this
/// impl block -- see each one's own doc comment for why.
...
// Both helpers above are called from methods on `Trap` and from the four
// free (`#[no_mangle]`) asm entry points below -- `Trap::` is always in
// scope in this file, so a fully-qualified path works from either call
// site without per-invocation `Self::` vs `Trap::` bookkeeping.
macro_rules! kassert {
    ($cond:expr, $func:expr, $msg:expr) => {
        if !($cond) {
            Trap::kassert_fail(line!(), $func, $msg)
        }
    };
}
...
impl Trap {
        ...
        unsafe {
            ...
            for i in 0..(NCPU as u64) {
                ...
                let cpu_i = (&raw mut crate::ipi::cpus)
                    .cast::<cpu_local>()
                    .add(i as usize);
                (*cpu_i).intr_stacks = Self::kirqstack(i) as *mut *mut c_void;
    ...
    pub(crate) extern "C" fn trapinithart() {
        ...
        Self::write_sscratch(intr_sp);
        Self::write_stvec(kernelvec as usize as u64);
    }
}
...
impl Trap {
    fn kerneltrap_dump_regs(tf: *mut trapframe) {
        ...
        unsafe {
            ...
            crate::kprintln!("gp: 0x{:x}", Self::read_gp());
        }
    }
    ...
    fn trap_panic_dump(tf: *mut trapframe, s0: u64) -> ! {
        ...
        unsafe {
            crate::kprintln!(
                ...
                (*tf).scause,
                Cs(Self::scause_to_str((*tf).scause).as_ptr()),
                (*tf).sepc,
                (*tf).stval,
            );
            (*tf).ra = (*tf).sepc;
...
pub extern "C" fn user_kirq_entrance(_ksp: u64, s0: u64) {
    ...
    unsafe {
        ...
        if IrqCore::do_irq(tf) < 0 {
            Trap::trap_panic_dump(tf, s0);
        }
    }
    Trap::exit_irq();

    if Trap::needs_resched() {
...
pub(crate) extern "C" fn usertrapret() {
    ...
    let (trapframe_base, satp) = unsafe {
        ...
        let satp = Self::make_satp((*(*p).vm).pagetable);
        (base, satp)
    };
...
pub extern "C" fn kerneltrap(sp: *mut trapframe, s0: u64) {
    if Trap::cpu_in_itr() {
        ...
        Trap::trap_panic_dump(sp, s0);
    }
    if Trap::intr_depth_postinc() != 0 {
        crate::kprint!(
