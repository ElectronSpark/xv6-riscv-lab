...
static int ramdisk_open(blkdev_t *blkdev) { return 0; }

static int ramdisk_release(blkdev_t *blkdev) { return 0; }
...
static int ramdisk_submit_bio(blkdev_t *blkdev, struct bio *bio) {
    ...
    bio_for_each_segment(&bvec, bio, &iter) {
        ...
        if (offset + bvec.len > ramdisk.size_bytes) {
            printf("ramdisk: access beyond end of device (offset=%lx, len=%d, "
                   "size=%lx)\n",
