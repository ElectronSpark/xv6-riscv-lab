...
uint32 __uart0_reg_shift =
    0; // 0 = 1-byte spacing, 2 = 4-byte spacing (common on SoCs)
uint32 __uart0_reg_io_width =
    1; // 1 = 8-bit access, 4 = 32-bit access (PXA UART)
...
// 2) FIFO reset (enable->clear->disable): flush stale RX/TX state; mirrors
// Linux PXA flow. 3) Read LSR/RHR/IIR/MSR: drains latched status so later
// enables do not fire immediately. 4) LCR=8N1: console framing expected by boot
// ROM/host; keeps parity/stop bits default. 5) MCR sets DTR/RTS and OUT2: OUT2
// is required on PXA to wire the IRQ line into the PLIC; without OUT2 we
// previously saw no UART interrupts and a stuck TX path. 6) FCR trigger: PXA
// (64-byte FIFO) uses 8-byte to cut interrupt rate; 16550 (16-byte) uses 1-byte
// for latency. Earlier, forcing 8-byte on 16550 caused sluggish echo. 7) Read
// status again after FIFO re-enable: ensure no pending conditions remain. 8)
// IER: enable RX; on PXA also RTOIE (RX timeout) plus UUE to power the block;
// TX is toggled dynamically when data exists. Historical missteps: using UART5
// (unaligned base) broke MMIO mapping; missing reg-shift/reg-io-width led to
// bad register offsets; omitting UUE left the PXA UART inert; omitting \r
...
int uartinit(void) {
    ...
    // RX trigger: 8-byte on PXA (64-byte FIFO to cut IRQ rate), 1-byte on 16550
    // for responsiveness
    if (__uart0_reg_io_width == 4)
    ...
    // Enable RX interrupts; PXA also needs RTOIE for RX timeout and UUE to
    // power the block
    if (__uart0_reg_io_width == 4) {
