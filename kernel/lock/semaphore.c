...
int sem_wait_timed(sem_t *sem, uint64 timeout_ms) {
    ...
    for (;;) {
        ...
        uint64 elapsed = sched_timer_now_ms() - start_ms;
        ...
        uint64 remaining_ms = timeout_ms - elapsed;
        if (remaining_ms == 0)
            remaining_ms = 1;

        struct timer_node tn = {0};
        int timer_ret = sched_timer_set(&tn, remaining_ms);
        if (timer_ret != 0) {
            /* Timer failed to install (e.g. TSC skew between CPUs
             * made expires <= current_tick).  Undo the decrement
             * and yield once so the caller sees a prompt timeout
