...
static int sem_wait_for(volatile int *ptr, int expected, int spin_loops) {
    ...
    if (__sync_add_and_fetch(&sem_t4_log_budget, 1) <= 8) {
        printf(
            "[sem][diag] wait_for timed out target=%p value=%d expected=%d\n",
...
static void sem_run_test1(void) {
    ...
    for (int i = 0; i < SEM_TEST_WAITERS; i++) {
        np = kthread_create("sem_t1", sem_test1_waiter, 0, 0,
                            KERNEL_STACK_ORDER);
...
static void sem_t4_consumer(uint64 a1, uint64 a2) {
    ...
    (void)a2;
    for (;;) {
        if (__atomic_load_n(&sem_t4_items_consumed, __ATOMIC_SEQ_CST) >=
                SEM_T4_TOTAL_ITEMS &&
            __atomic_load_n(&sem_t4_producers_done, __ATOMIC_SEQ_CST) >=
                SEM_T4_PRODUCERS)
            break;
        ...
        if (sem_error_flag &&
            __sync_add_and_fetch(&sem_t4_log_budget, 1) <= 8) {
            if (value < 0 || value >= SEM_T4_TOTAL_ITEMS) {
                printf(
                    "[sem][T4][cons] out-of-range value=%d tail=%d head=%d\n",
                    value, tail,
                    __atomic_load_n(&sem_t4_head, __ATOMIC_SEQ_CST));
            } else {
...
static void sem_run_test4(void) {
    ...
    for (int i = 0; i < SEM_T4_PRODUCERS; i++) {
        np = kthread_create("sem_prod", sem_t4_producer, 0, 0,
                            KERNEL_STACK_ORDER);
        if (IS_ERR_OR_NULL(np))
            sem_error_flag = 1;
        else
    ...
    for (int i = 0; i < SEM_T4_CONSUMERS; i++) {
        np = kthread_create("sem_cons", sem_t4_consumer, 0, 0,
                            KERNEL_STACK_ORDER);
        if (IS_ERR_OR_NULL(np))
            sem_error_flag = 1;
        else
    ...
    if (sem_wait_for(&sem_t4_producers_done, SEM_T4_PRODUCERS, 400000) != 0)
        ...
        for (int i = 0; i < SEM_T4_CONSUMERS; i++) {
            if (sem_post(&sem_t4_full) != 0 &&
                __sync_add_and_fetch(&sem_t4_log_budget, 1) <= 8) {
                printf(
                    "[sem][T4] failed to post wake sentinel for consumers\n");
            }
        }
    }
...
void semaphore_launch_tests(void) {
    struct thread *np = NULL;
    np = kthread_create("semaphore_test_master", semaphore_test_master, 0, 0,
                        KERNEL_STACK_ORDER);
    if (IS_ERR_OR_NULL(np)) {
