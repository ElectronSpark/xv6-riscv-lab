...
/*
 * On SMP the lightweight-protection sections must exclude other CPUs as
 * well as local interrupts.  A single global spinlock + IRQ save/restore
 * provides the necessary mutual exclusion for all SYS_ARCH_PROTECT /
 * SYS_ARCH_UNPROTECT regions (pbuf ref-counts, memp free-lists, etc.).
 */
static spinlock_t lwip_prot_lock = SPINLOCK_INITIALIZED("lwip_prot");

sys_prot_t sys_arch_protect(void)
{
    return spin_lock_irqsave(&lwip_prot_lock);
}

void sys_arch_unprotect(sys_prot_t pval)
{
    spin_unlock_irqrestore(&lwip_prot_lock, pval);
}
...
/* xv6: yield the current timeslice without using the timer subsystem. */
...
#define MAX_LWIP_THREAD_SEMS 128
...
void sys_arch_netconn_sem_alloc(void)
{
    ...
    spin_unlock(&lwip_thread_sem_lock);
    printf("lwip: too many threads using netconn API (max=%d)\n",
           MAX_LWIP_THREAD_SEMS);
}
