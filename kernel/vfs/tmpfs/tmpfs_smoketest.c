...
// Helper: lookup a child inode by name and bump refcount; caller must
// vfs_iput()
static struct vfs_inode *tmpfs_fetch_inode(struct vfs_inode *dir,
...
// Run a small, inode-only tmpfs smoke test: create nested directories, files,
// symlinks, perform lookups, moves, and readlinks. This is debug-only
// scaffolding invoked at init.
void tmpfs_run_inode_smoketest(void) {
    ...
    if (ret != 0) {
        ...
        struct vfs_inode *tmp =
            tmpfs_fetch_inode(subdir, file_a_name, file_a_len);
        if (!IS_ERR_OR_NULL(tmp)) {
            vfs_ilock(tmp);
            printf("inode_smoketest: " PASS
                   " linked /%s/%s -> /%s/%s nlink=%u\n",
                   subdir_name, file_a_link_name, subdir_name, file_a_name,
                   tmp->n_links);
            vfs_iunlock(tmp);
            vfs_iput(tmp);
        } else {
    ...
    if (IS_ERR_OR_NULL(sym_b)) {
        ...
        vfs_ilock(sym_b);
        printf("inode_smoketest: " PASS
               " symlink /%s/%s/%s -> %s ino=%lu nlink=%u\n",
               subdir_name, nested_name, symlink_b_name, symlink_b_target,
               sym_b->ino, sym_b->n_links);
        vfs_iunlock(sym_b);
        vfs_iput(sym_b);
        sym_b = NULL;
    }
    ...
    if (ret == 0) {
        struct vfs_inode *tmp =
            tmpfs_fetch_inode(nested, file_b_name, file_b_len);
        if (!IS_ERR_OR_NULL(tmp)) {
            vfs_ilock(tmp);
            printf("inode_smoketest: " PASS
                   " ilookup /%s/%s/%s -> ino=%lu nlink=%u\n",
    ...
    ret =
        vfs_move(nested, &old_dentry, subdir, file_b_new_name, file_b_new_len);
    if (ret != 0) {
    ...
    if (!IS_ERR_OR_NULL(tmp_sym)) {
        ...
        if (ret >= 0) {
            vfs_ilock(tmp_sym);
            printf("inode_smoketest: " PASS
                   " readlink /%s/%s/%s -> %s len=%d nlink=%u\n",
...
// Run truncate smoke tests: grow/shrink across embedded, direct, indirect,
// double indirect layers. Uses at most ~1000 blocks (~4MB) to stay well under
// 64MB limit.
void tmpfs_run_truncate_smoketest(void) {
    ...
    if (ret != 0) {
        ...
        printf("truncate_smoketest: " PASS
               " grow embedded 0->100 bytes, size=%llu\n",
    ...
    if (ret != 0) {
        ...
        printf("truncate_smoketest: " PASS
               " grow to direct 5 blocks, size=%llu n_blocks=%lu\n",
    ...
    if (ret != 0) {
        ...
        printf("truncate_smoketest: " PASS
               " shrink to embedded 50 bytes, size=%llu n_blocks=%lu\n",
               (unsigned long long)test_file->size, test_file->n_blocks);
    }
    ...
    vfs_iunlock(test_file);
    if (ret != 0) {
        printf("truncate_smoketest: " FAIL
               " grow zero to 1000 pages, errno=%d\n",
               ret);
    } else {
        printf("truncate_smoketest: " PASS
               " grow zero to 1000 pages, size=%llu\n",
               (unsigned long long)test_file->size);
    }
...
void tmpfs_run_namei_smoketest(void) {
    ...
    if (IS_ERR_OR_NULL(result)) {
        ...
        printf("namei_smoketest: " FAIL
               " namei(\"%s\") wrong ino=%lu expected=%lu\n",
    ...
    if (result == ERR_PTR(-ENOENT)) {
        printf("namei_smoketest: " PASS
               " namei(\"%s\") -> ENOENT as expected\n",
               path8);
...
static bool iter_mark_seen(const char *name, struct iter_expect *tbl,
                           int tbl_cnt, const char *tag, bool *dup_out) {
...
// allow_extra: if true, unexpected entries are warnings; if false, they are
// failures
static void tmpfs_iter_and_fetch_ex(const char *tag, struct vfs_inode *dir,
    ...
    for (;;) {
        ...
        printf("dir_iter_mount: ITER %s step=%d after dir_iter ret=%d "
               "cookies=%ld\n",
        ...
        if (IS_ERR_OR_NULL(ent)) {
            ...
            printf("dir_iter_mount: entry %s name=%s ino=%lu fetched_ino=%lu "
                   "sb_match=%s locked=%s\n",
                   tag, dentry.name ? dentry.name : "(null)", dentry.ino,
                   ent->ino, ent->sb == dentry.sb ? "yes" : "no",
                   locked ? "yes" : "no");
...
// Mount a second tmpfs on a subdirectory, populate it, and iterate with
// vfs_dir_iter
void tmpfs_run_dir_iter_mount_smoketest(void) {
    ...
    // --- Test 1: empty FS local root (freshly mounted, no content yet) ---
    {
        ...
        tmpfs_iter_and_fetch_ex("mnt_root_empty", mnt_root, &iter, exp, 2,
                                false);
    }
    ...
    // Temporarily chroot to the empty mounted fs root
    {
        ...
        if (old_root != NULL) {
            ...
            if (ret != 0) {
                ...
                tmpfs_iter_and_fetch_ex("process_root_empty", mnt_root, &iter,
                                        exp, 2, false);
    ...
    // --- Test 3: non-empty FS local root (mnt_root has file + subdir) ---
    {
        ...
        tmpfs_iter_and_fetch_ex("mnt_root_nonempty", mnt_root, &iter, exp, 4,
                                false);
    }
    ...
    // --- Test 4: empty ordinary subdir (mnt_subdir is empty) ---
    {
        ...
        tmpfs_iter_and_fetch_ex("subdir_empty", mnt_subdir, &iter, exp, 2,
                                false);
    }
    ...
    if (IS_ERR_OR_NULL(tmp)) {
        ...
        tmpfs_iter_and_fetch_ex("subdir_nonempty", mnt_subdir, &iter, exp, 3,
                                false);

        ret = vfs_unlink(mnt_subdir, nested_file, nested_file_len);
        if (ret != 0) {
            printf("dir_iter_mount: " WARN
                   " cleanup unlink nested_file errno=%d\n",
                   ret);
        }
    }

    // --- Test 6: non-empty process root (has iter_mount_dir only - fresh
    // tmpfs) --- Temporarily chroot to the populated tmpfs root
    {
        struct vfs_inode *old_root = vfs_curroot();
        if (old_root != NULL) {
            ret = vfs_chroot(root);
            if (ret != 0) {
                printf("dir_iter_mount: " FAIL
                       " chroot to root for process_root_nonempty errno=%d\n",
                       ret);
                vfs_iput(old_root);
            } else {
                struct iter_expect exp[] = {
                ...
                tmpfs_iter_and_fetch_ex("process_root_nonempty", root, &iter,
                                        exp, 3, false);
                // Restore original root
                ret = vfs_chroot(old_root);
                if (ret != 0) {
    ...
    // These tests need to chroot to root (tmpfs) so that root IS the process
    // root
    printf("dir_iter_mount: BEGIN ilookup_dotdot tests\n");
    {
        ...
        if (old_root == NULL) {
            printf("dir_iter_mount: " FAIL
                   " vfs_curroot returned NULL for ilookup tests\n");
        } else {
            ret = vfs_chroot(root);
            if (ret != 0) {
                printf("dir_iter_mount: " FAIL
                       " chroot to root for ilookup tests errno=%d\n",
                       ret);
                vfs_iput(old_root);
            } else {
                // Test 7: ".." at process root -> should return self
                {
                    ...
                    if (ret != 0) {
                        printf("dir_iter_mount: " FAIL
                               " ilookup(..) at process_root errno=%d\n",
                               ret);
                    } else if (d.ino != root->ino) {
                        printf("dir_iter_mount: " FAIL
                               " ilookup(..) at process_root wrong ino=%lu "
                               "expected=%lu\n",
                               d.ino, root->ino);
                    } else {
                        printf("dir_iter_mount: " PASS
                               " ilookup(..) at process_root -> self ino=%lu\n",
                               d.ino);
                    }
                    vfs_release_dentry(&d);
                }

                // Test 8: ".." at FS local root (mnt_root) -> should cross
                // mount and return root
                {
                    struct vfs_dentry d = {0};
                    ret = vfs_ilookup(mnt_root, &d, "..", 2);
                    if (ret != 0) {
                        printf("dir_iter_mount: " FAIL
                               " ilookup(..) at mnt_root errno=%d\n",
                               ret);
                    } else {
                        // Must fetch the actual inode to compare across
                        // superblock boundaries
                        struct vfs_inode *parent = vfs_get_dentry_inode(&d);
                        if (IS_ERR_OR_NULL(parent)) {
                            printf(
                                "dir_iter_mount: " FAIL
                                " ilookup(..) at mnt_root get_inode failed\n");
                        } else if (parent != root) {
                            printf("dir_iter_mount: " FAIL
                                   " ilookup(..) at mnt_root wrong inode (got "
                                   "ino=%lu sb=%p, expected ino=%lu sb=%p)\n",
                                   parent->ino, parent->sb, root->ino,
                                   root->sb);
                            vfs_iput(parent);
                        } else {
                            printf(
                                "dir_iter_mount: " PASS
                                " ilookup(..) at mnt_root -> parent ino=%lu\n",
                                parent->ino);
                            vfs_iput(parent);
                        }
                    }
                ...
                // Test 9: ".." at ordinary subdir (mnt_subdir) -> should return
                // mnt_root
                {
                    struct vfs_dentry d = {0};
                    ...
                    if (ret != 0) {
                        printf("dir_iter_mount: " FAIL
                               " ilookup(..) at mnt_subdir errno=%d\n",
                               ret);
                    } else if (d.ino != mnt_root->ino || d.sb != mnt_root->sb) {
                        printf("dir_iter_mount: " FAIL
                               " ilookup(..) at mnt_subdir wrong ino=%lu "
                               "expected=%lu\n",
                               d.ino, mnt_root->ino);
                    } else {
                        printf("dir_iter_mount: " PASS
                               " ilookup(..) at mnt_subdir -> parent ino=%lu\n",
                               d.ino);
                    }
                    vfs_release_dentry(&d);
                }
                ...
                // Test 11: "." at FS local root -> should return self
                {
                    ...
                    if (ret != 0) {
                        ...
                        printf("dir_iter_mount: " FAIL
                               " ilookup(.) at mnt_root wrong ino=%lu "
                               "expected=%lu\n",
                               d.ino, mnt_root->ino);
                    } else {
                        printf("dir_iter_mount: " PASS
                               " ilookup(.) at mnt_root -> self ino=%lu\n",
                               d.ino);
                    }
                    vfs_release_dentry(&d);
                }
                ...
                // Test 12: "." at ordinary subdir -> should return self
                {
                    ...
                    if (ret != 0) {
                        ...
                        printf("dir_iter_mount: " PASS
                               " ilookup(.) at mnt_subdir -> self ino=%lu\n",
                               d.ino);
                    }
                    vfs_release_dentry(&d);
                }
                ...
                if (ret != 0) {
                    printf("dir_iter_mount: " FAIL
                           " restore chroot after ilookup tests errno=%d\n",
                           ret);
                }
                vfs_iput(old_root);
            }
...
cleanup_mount:
    ...
    if (mp != NULL && mp->mnt_sb != NULL) {
        ...
        // On success, vfs_unmount already unlocked/freed child_root, child_sb,
        // mp, and mp->sb
        vfs_mount_unlock();
    }
...
cleanup_test1_mp:
    ...
    // Test 2a: Read/write should still work on orphan file
    {
        ...
        if (pos != 0) {
            ...
            char read_buf[64] = {0};
            ssize_t bytes_read =
                vfs_fileread(open_file, read_buf, test_data_len, false);
            if (bytes_read != (ssize_t)test_data_len) {
                ...
                printf("lazy_unmount: " PASS
                       " test2a orphan file read works\n");
            }
        }
    }
...
cleanup_test2_mp:
    ...
    // Lazy unmount while holding subdir reference
    {
        ...
        vfs_mount_unlock();
        printf("lazy_unmount: " PASS
               " test3 lazy unmount with held directory\n");
    }
    ...
    subdir = NULL;
    printf("lazy_unmount: " PASS
           " test3 orphan directory released, cleanup complete\n");
...
void tmpfs_run_file_ops_smoketest(void) {
    ...
    if (pos != (loff_t)(test_data_len - 10)) {
        printf("file_ops_smoketest: " FAIL
               " lseek SEEK_CUR -10, expected %lu got %lld\n",
    ...
    if (pos != (loff_t)test_data_len) {
        printf("file_ops_smoketest: " FAIL
               " lseek SEEK_END 0, expected %lu got %lld\n",
    ...
    if (bytes_read != (ssize_t)test_data_len ||
        memcmp(read_buf, test_data, test_data_len) != 0) {
        printf("file_ops_smoketest: " FAIL " re-read after close mismatch\n");
    } else {
        printf("file_ops_smoketest: " PASS
               " re-read after close, data persisted\n");
    }
    ...
    if (IS_ERR(file)) {
        ret = PTR_ERR(file);
        printf("file_ops_smoketest: " FAIL
               " open for multi-block test, errno=%d\n",
               ret);
        goto cleanup;
    }
    ...
    while (total_written < target_size) {
        ...
        if (to_write > sizeof(write_buf))
            to_write = sizeof(write_buf);
        written = vfs_filewrite(file, write_buf, to_write, false);
        if (written <= 0) {
            printf("file_ops_smoketest: " FAIL
                   " multi-block write at %lu, errno=%ld\n",
    ...
    if (bytes_read == 50 && read_buf[0] == 'A') {
        ...
        printf(
            "file_ops_smoketest: " FAIL
            " read from middle of second page (got %ld bytes, first char=%d)\n",
...
static bool verify_pattern(const char *buf, size_t len, loff_t offset) {
    for (size_t i = 0; i < len; i++) {
        ...
        if (buf[i] != expected) {
            printf("  pattern mismatch at offset %lld+%lu: got 0x%02x expected "
                   "0x%02x\n",
                   (long long)offset, (unsigned long)i, (unsigned char)buf[i],
...
static bool write_and_verify(struct vfs_file *file, loff_t offset, size_t len,
                             char *write_buf, char *read_buf,
                             const char *desc) {
    // Seek and write
    loff_t pos = vfs_filelseek(file, offset, SEEK_SET);
    if (pos != offset) {
        printf("dindirect_smoketest: " FAIL
               " %s seek to %lld failed (got %lld)\n",
    ...
    if (written != (ssize_t)len) {
        printf("dindirect_smoketest: " FAIL
               " %s write %lu bytes at %lld failed (got %ld)\n",
    ...
    printf("dindirect_smoketest: " PASS " %s (offset=%lld, len=%lu)\n", desc,
           (long long)offset, (unsigned long)len);
    return true;
}
...
void tmpfs_run_double_indirect_smoketest(void) {
    ...
    // Use static buffers for pattern writes (up to 3 pages for cross-boundary
    // tests)
    ...
    write_and_verify(file, 0, 100, write_buf, read_buf,
                     "start of file (embedded)");

    // Test 1b: Write at last byte of direct layer
    write_and_verify(file, DIRECT_END - 100, 100, write_buf, read_buf,
                     "end of direct layer");

    // Test 1c: Write crossing direct->indirect boundary
    write_and_verify(file, DIRECT_END - 50, 100, write_buf, read_buf,
                     "cross direct->indirect boundary");

    // Test 1d: Write at start of indirect layer (already covered by 1c
    // extension)
    write_and_verify(file, DIRECT_END, 100, write_buf, read_buf,
                     "start of indirect layer");

    // Test 1e: Write at end of indirect layer (extends file through indirect)
    write_and_verify(file, INDIRECT_END - 100, 100, write_buf, read_buf,
                     "end of indirect layer");

    // Test 1f: Write crossing indirect->double_indirect boundary
    write_and_verify(file, INDIRECT_END - 50, 100, write_buf, read_buf,
                     "cross indirect->dindirect boundary");

    // Test 1g: Write at start of double indirect layer
    write_and_verify(file, INDIRECT_END, 100, write_buf, read_buf,
                     "start of double indirect layer");
    ...
    printf("dindirect_smoketest: extending file through double indirect via "
           "truncate...\n");
    ...
    if (IS_ERR(file)) {
        ret = PTR_ERR(file);
        printf("dindirect_smoketest: " FAIL
               " reopen after 10MB extend, errno=%d\n",
               ret);
        goto cleanup;
    }

    // Test 1h: Write deep in double indirect (10MB offset - within allocated
    // range)
    write_and_verify(file, 10 * 1024 * 1024 - 100, 100, write_buf, read_buf,
                     "at 10MB");
    ...
    if (IS_ERR(file)) {
        ret = PTR_ERR(file);
        printf("dindirect_smoketest: " FAIL
               " reopen after 32MB extend, errno=%d\n",
               ret);
        goto cleanup;
    }

    // Test 1j: Write at 32MB
    write_and_verify(file, 32 * 1024 * 1024 - 100, 100, write_buf, read_buf,
                     "at 32MB");

    // Test 1k: Large write spanning multiple pages in double indirect (at 20MB)
    write_and_verify(file, 20 * 1024 * 1024, PAGE_SIZE + 500, write_buf,
                     read_buf, "multi-page write in dindirect (20MB)");
    ...
    if (IS_ERR(file)) {
        ret = PTR_ERR(file);
        printf("dindirect_smoketest: " FAIL
               " reopen after 64MB extend, errno=%d\n",
               ret);
        goto cleanup;
    }

    // Test 1l: Write near 64MB limit
    write_and_verify(file, TARGET_64MB - 100, 100, write_buf, read_buf,
                     "near 64MB limit");
    ...
    for (size_t i = 0; i < sizeof(verify_points) / sizeof(verify_points[0]);
         i++) {
    ...
    if (all_verified) {
        printf("dindirect_smoketest: " PASS
               " all persistence verifications passed\n");
    }
    ...
    if (ret == 0 && test_inode->size == INDIRECT_END) {
        printf("dindirect_smoketest: " PASS
               " grow to exact indirect end (%lld)\n",
               (long long)INDIRECT_END);
    } else {
    ...
    if (ret == 0) {
        ...
        if (!IS_ERR(file)) {
            ...
            if (bytes_read == 100) {
                ...
                if (all_zero) {
                    ...
                    printf("dindirect_smoketest: " WARN
                           " gap not zero-filled\n");
                }
            }
            vfs_fput(file);
        ...
        printf("dindirect_smoketest: " FAIL
               " truncate-extend to 8MB, errno=%d\n",
               ret);
    }
    ...
    if (!IS_ERR(file)) {
        ...
        loff_t span_start =
            DIRECT_END - PAGE_SIZE; // Start 1 page before boundary
