...
int vfs_ilookup(struct vfs_inode *dir, struct vfs_dentry *dentry,
    ...
    KSTATS_PROFILE_INC(g_vfs_lookup_calls);
    ...
    uint64 lookup_start = profile ? r_time() : 0;
    if (profile)
        __atomic_add_fetch(&g_vfs_lookup_driver_calls, 1, __ATOMIC_RELAXED);
    ret = dir->ops->lookup(dir, dentry, name, name_len);
    if (profile)
        __atomic_add_fetch(&g_vfs_lookup_driver_ticks,
                           r_time() - lookup_start, __ATOMIC_RELAXED);
    if (ret == 0) {
...
int vfs_chdir(struct vfs_inode *new_cwd) {
    ...
    if (new_cwd == vfs_inode_deref(&current->fs->cwd)) {
        // No change
        return 0;
    }
...
int vfs_chroot(struct vfs_inode *new_root) {
    ...
    if (new_root == vfs_inode_deref(&current->fs->rooti)) {
        // No change
        return 0;
    }
    struct vfs_inode_ref ref = {0};
    ...
    vfs_idup(new_root);
    vfs_struct_lock(current->fs);
    old = current->fs->rooti;
    current->fs->rooti = ref;
    vfs_struct_unlock(current->fs);
    vfs_inode_put_ref(&old);
    return 0;
}
