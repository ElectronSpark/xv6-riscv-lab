                              ...
                              bool user)
{
    ...
    while ((size_t)total < count) {
        ...
        if (got > 0) {
            /* Wake peer's writers since we freed space */
            tq_wakeup_all(&peer->wr_queue, 0, 0);
            if (peer->file)
                vfs_file_knote_notify(peer->file, EVFILT_WRITE, 0);
        }
    ...
    /* Notify kqueue: after reading, peer's write-side may become writable */
    if (total > 0 && sk->file)
        vfs_file_knote_notify(sk->file, EVFILT_READ, 0);
                               ...
                               size_t count, bool user)
{
    ...
    while ((size_t)total < count) {
        ...
        while (wrote < chunk) {
            ...
            if (w > 0) {
                ...
                if (peer->file)
                    vfs_file_knote_notify(peer->file, EVFILT_READ, 0);
            }
    ...
    if (total > 0 && sk->file)
        vfs_file_knote_notify(sk->file, EVFILT_WRITE, 0);
...
static int unix_file_release(struct vfs_inode *inode, struct vfs_file *file)
{
     ...
     * If peer->peer still points to us, clear it and mark EOF.
     */
    if (peer != NULL) {
...
int unix_sock_connect(int fd, uint64 uaddr, int addrlen)
{
    ...
    if (target->file)
        vfs_file_knote_notify(target->file, EVFILT_READ, 0);
