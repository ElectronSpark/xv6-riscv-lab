...
/// Zero-sized namespace for this file's current-process-keyed syscall
/// orchestration: fd/fdtable/`fs_struct` accessors and every `sys_vfs_*`
/// dispatcher's `*_inner` helper. No natural C struct backs "the current
/// syscall" -- same precedent as `vfs/fs.rs`'s `Vfs` marker (mount-mutex
/// primitives) and `mm/kalloc.rs`'s `Kmem`.
...
impl Sys {
    fn current_fdtable() -> *mut vfs_fdtable {
        // SAFETY: `Sys::current()` is the live running thread.
        unsafe { (*Sys::current()).fdtable }
    }
}
...
impl Sys {
    fn stat_inner() -> KResult<()> {
        ...
        let mut kst: stat = unsafe { core::mem::zeroed() };
        let ret = VfsInode::vfs_inode_stat_fallback(IRef::as_ptr(&inode), &mut kst);
        if ret != 0 {
            return Err(Errno::Raw(ret));
        }
...
pub(crate) extern "C" fn sys_vfs_stat() -> u64 {
    ret64(result_to_neg_errno(Sys::stat_inner()))
}
...
impl Sys {
    fn lstat_inner() -> KResult<()> {
        ...
        let mut kst: stat = unsafe { core::mem::zeroed() };
        let ret = VfsInode::vfs_inode_stat_fallback(inode, &mut kst);
...
pub(crate) extern "C" fn sys_vfs_lstat() -> u64 {
    ret64(result_to_neg_errno(Sys::lstat_inner()))
}
...
pub(crate) extern "C" fn sys_vfs_access() -> u64 {
    ret64(result_to_neg_errno(Sys::access_inner()))
}
...
pub(crate) extern "C" fn sys_vfs_mkdir() -> u64 {
    ret64(result_to_neg_errno(Sys::mkdir_inner()))
}
...
pub(crate) extern "C" fn sys_vfs_mknod() -> u64 {
    ret64(result_to_neg_errno(Sys::mknod_inner()))
}
...
/// Mirrors the C `Sys::vfs_make_absolute_path()`.
impl Sys {
    fn vfs_make_absolute_path(relpath: &[c_char], relpath_len: c_int, abspath: &mut [c_char; MAXPATH]) -> c_int {
        ...
        let fs = Sys::current_fs();
        let (cwd, root) = {
            let _g = Sys::fs_lock(fs);
...
impl Sys {
    fn symlink_inner() -> KResult<()> {
        ...
        let abs_len = Sys::vfs_make_absolute_path(&target, n1, &mut abs_target);
        if abs_len < 0 {
...
pub(crate) extern "C" fn sys_vfs_symlink() -> u64 {
    ret64(result_to_neg_errno(Sys::symlink_inner()))
}
...
pub(crate) extern "C" fn sys_vfs_chdir() -> u64 {
    ret64(result_to_neg_errno(Sys::chdir_inner()))
}
...
/// extern). Mirrors the C `Vfs::vfs_mount_path()`.
impl Vfs {
    pub(crate) fn vfs_mount_path(
        fstype: *const c_char,
        target: *const c_char,
        target_len: c_int,
        source: *const c_char,
...
impl Sys {
    fn mount_inner() -> KResult<c_int> {
        ...
        Ok(Vfs::vfs_mount_path(fstype.as_ptr(), target.as_ptr(), n2, source.as_ptr(), n1))
    }
}

pub(crate) extern "C" fn sys_mount() -> u64 {
    match Sys::mount_inner() {
        Ok(v) => ret64(v),
        Err(e) => ret64(e.neg()),
    }
}
...
/// it. Mirrors the C `Vfs::vfs_umount_path()`.
impl Vfs {
    pub(crate) fn vfs_umount_path(target: *const c_char, target_len: c_int) -> c_int {
        ...
        // SAFETY: `mounted_root` is non-null (checked above); `is_local_root`
        // (pub(crate) on `VfsInode`) reads only set-once fields.
        if !unsafe { &*mounted_root }.is_local_root() {
...
impl Sys {
    fn umount_inner() -> KResult<c_int> {
        ...
        Ok(Vfs::vfs_umount_path(target.as_ptr(), n))
    }
}

pub(crate) extern "C" fn sys_umount() -> u64 {
    match Sys::umount_inner() {
        Ok(v) => ret64(v),
        Err(e) => ret64(e.neg()),
    }
}
...
pub(crate) extern "C" fn sys_dumpinode() -> u64 {
    ret64(result_to_neg_errno(Sys::dumpinode_inner()))
}
...
pub(crate) extern "C" fn sys_statfs() -> u64 {
    ret64(result_to_neg_errno(Sys::statfs_inner()))
}
