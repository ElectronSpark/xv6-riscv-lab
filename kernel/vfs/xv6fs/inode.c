 ...
 * 1. vfs_superblock rwsem (held by VFS layer for create/mkdir/unlink/etc)
...
static int __xv6fs_dirlink(struct xv6fs_superblock *xv6_sb,
                           struct xv6fs_inode *dp, const char *name,
                           uint inum) {
    struct dirent de;
...
struct vfs_inode *xv6fs_create(struct vfs_inode *dir, mode_t mode,
    ...
    struct xv6fs_inode *ip =
        container_of(new_inode, struct xv6fs_inode, vfs_inode);
    ip->dev = dp->dev;
...
int xv6fs_unlink(struct vfs_dentry *dentry, struct vfs_inode *target) {
    ...
    struct xv6fs_superblock *xv6_sb =
        container_of(dentry->sb, struct xv6fs_superblock, vfs_sb);

    struct xv6fs_inode *dp =
        container_of(dentry->parent, struct xv6fs_inode, vfs_inode);
    struct dirent de;
    uint off;
    ...
    for (off = 0; off < dentry->parent->size; off += sizeof(de)) {
        ...
        if (dentry->name_len == strnlen(de.name, DIRSIZ) &&
            ...
            struct xv6fs_inode *tip =
                container_of(target, struct xv6fs_inode, vfs_inode);
            xv6fs_iupdate(tip);
...
int xv6fs_rmdir(struct vfs_dentry *dentry, struct vfs_inode *target) {
    ...
    if (ret == 0) {
        // Decrement parent's link count (for the ".." entry in the removed
        // directory) This mirrors xv6fs_mkdir which increments parent's n_links
        // when creating a subdir
        struct xv6fs_inode *dp =
            container_of(dentry->parent, struct xv6fs_inode, vfs_inode);
