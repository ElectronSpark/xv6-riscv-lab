 ...
 * 1. They require BATCHED transactions for large files (multiple begin/end
 * cycles)
 ...
 */
static uint __xv6fs_balloc(struct xv6fs_superblock *xv6_sb, uint dev,
                           uint hint) {
...
static void __xv6fs_itrunc_ind(struct xv6fs_superblock *xv6_sb, uint *entry,
                               uint dev) {
    __xv6fs_itrunc_ind_partial(xv6_sb, entry, dev, 0);
}

// Maximum blocks to free per transaction to stay within log limits
// Each bfree modifies the bitmap (1 write) so we can free ~MAXOPBLOCKS/2 blocks
// per tx
...
// This function may call xv6fs_end_op and xv6fs_begin_op internally to commit
// batches. When this function returns, a transaction is still active (caller
// should call xv6fs_end_op).
...
static int __xv6fs_truncate_partial(struct xv6fs_inode *ip, uint first_block) {
    ...
    if (first_block <= XV6FS_NDIRECT) {
        ...
        __xv6fs_itrunc_ind_partial(xv6_sb, &ip->addrs[XV6FS_NDIRECT], dev,
                                   ind_start);
    }
