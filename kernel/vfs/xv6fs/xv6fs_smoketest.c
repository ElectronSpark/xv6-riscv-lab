...
// vfs_iput()
static struct vfs_inode *xv6fs_fetch_inode(struct vfs_inode *dir,
...
void xv6fs_run_inode_smoketest(void) {
    ...
    if (root == NULL) {
        printf("xv6fs_inode_test: " WARN
               " xv6fs not mounted at /root, skipping\n");
        return;
    }
    ...
    if (ret != 0) {
        ...
        struct vfs_inode *tmp =
            xv6fs_fetch_inode(testdir, file_a_name, file_a_len);
    ...
    // Create a symlink
    sym = vfs_symlink(testdir, 0777, sym_name, sym_len, sym_target,
                      sym_target_len);
...
void xv6fs_run_file_ops_smoketest(void) {
    ...
    const char *test_data =
        "Hello, xv6fs! This is a test message for file operations.";
    ...
    // Read data
    ssize_t bytes_read =
        vfs_fileread(file, read_buf, sizeof(read_buf) - 1, false);
    if (bytes_read < 0) {
...
void xv6fs_run_truncate_smoketest(void) {
    ...
    if (ret != 0) {
        printf("xv6fs_truncate: " FAIL
               " partial shrink to 2 blocks, errno=%d\n",
               ret);
    } else {
    ...
    if (ret != 0) {
        printf("xv6fs_truncate: " FAIL
               " partial shrink to 500 bytes, errno=%d\n",
               ret);
    } else {
        printf("xv6fs_truncate: " PASS
...
void xv6fs_run_namei_smoketest(void) {
    ...
    if (root == NULL) {
        printf("xv6fs_namei: " WARN " xv6fs not mounted at /root, skipping\n");
        return;
    }
    ...
    if (IS_ERR(result)) {
        ...
        printf("xv6fs_namei: " FAIL
               " namei(\"%s\") should fail but got ino=%lu\n",
               path6, result->ino);
        vfs_iput(result);
    }
    result = NULL;
...
static bool xv6fs_iter_mark_seen(const char *name,
                                 struct xv6fs_iter_expect *tbl, int tbl_cnt,
                                 bool *dup_out) {
...
void xv6fs_run_dir_iter_smoketest(void) {
    ...
    printf("xv6fs_dir_iter: Test 2 - populated directory\n");
    {
        struct xv6fs_iter_expect exp[] = {
            ...
            {"file1", true, false},  {"file2", true, false},
            {"subdir", true, false},
        };
        int exp_count = 5;
        bool ok = true;
    ...
    printf("xv6fs_dir_iter: Test 3 - fetch inodes during iteration\n");
    {
        ...
        if (ok && fetched == 5) {
            printf("xv6fs_dir_iter: " PASS
                   " fetched all 5 inodes successfully\n");
        }
        vfs_release_dentry(&dentry);
    }
...
void xv6fs_run_large_file_smoketest(void) {
    ...
    for (int block = 0; block < 10; block++) {
        ...
        for (int i = 0; i < BSIZE; i++) {
            if (read_buf[i] != expected) {
                printf("xv6fs_largefile: " FAIL
                       " block %d byte %d: got 0x%02x expected 0x%02x\n",
                       block, i, (unsigned char)read_buf[i],
    ...
    if (new_pos == 5 * BSIZE) {
        ...
        ssize_t bytes_read = vfs_fileread(file, read_buf, BSIZE, false);
        if (bytes_read == BSIZE &&
            read_buf[0] == 'F') { // Block 5 has pattern 'F'
            printf("xv6fs_largefile: " PASS
                   " seek to block 5 and read verified\n");
        } else {
            printf("xv6fs_largefile: " FAIL
                   " seek+read: read=%ld first_byte=0x%02x\n",
