...
static int extent_keys_cmp(uint64 key1, uint64 key2)
{
    ...
    if (ext1->start < ext2->start) return -1;
    if (ext1->start > ext2->start) return 1;
...
bcache_alloc_extent(struct xv6fs_block_cache *bc)
{
...
static void
bcache_free_extent(struct xv6fs_block_cache *bc, struct free_extent *ext)
{
    slab_free(ext);
}
 ...
 */
static struct free_extent *
bcache_find_extent_le(struct xv6fs_block_cache *bc, uint32 blockno)
{
 ...
 */
static struct free_extent *
bcache_find_extent_ge(struct xv6fs_block_cache *bc, uint32 blockno)
{
    struct rb_node *node = bc->extent_tree.node;
    struct free_extent *best = 0;
 ...
 */
static void
bcache_insert_extent(struct xv6fs_block_cache *bc, uint32 start, uint32 length)
{
 ...
 */
static uint32
bcache_alloc_from_extent(struct xv6fs_block_cache *bc, struct free_extent *ext)
{
...
xv6fs_bcache_find_free_block(struct xv6fs_superblock *xv6_sb, uint32 *blockno_out)
{
    ...
    if (!ext) {
        ext = container_of(rb_first_node(&bc->extent_tree), 
                          struct free_extent, rb_node);
    }
                                   ...
                                   uint32 hint, uint32 *blockno_out)
{
    ...
    ext = container_of(rb_last_node(&bc->extent_tree), 
                      struct free_extent, rb_node);
    *blockno_out = bcache_alloc_from_extent(bc, ext);
    spin_unlock(&bc->lock);
    return 0;
...
xv6fs_bcache_init(struct xv6fs_superblock *xv6_sb)
{
    ...
    bc->initialized = 1;
    printf("xv6fs: block cache initialized: %d data blocks, %d free in %d extents\n",
