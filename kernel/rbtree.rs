...
//!
//! KERNEL-OO: every free fn below relocates onto `impl RbNode`/`impl
//! RbRoot` (both types owned by `crate::bintree`; Rust permits inherent
//! impls on a foreign-module-but-same-crate type from any file). Raw
//! params, no `&self` — same "Freeze + intrusively linked, mutated
//! cross-hart under external locking" rule as `bintree.rs`.

#![allow(non_camel_case_types)]

use core::ptr;

pub(crate) use crate::bintree::{RbNode, RbRoot};
...
impl RbNode {
    ...
    /// `node` must be null or point to a live `rb_node`.
    #[inline(always)]
    unsafe fn dye_black(node: *mut RbNode) {
        if node.is_null() {
            return;
        }
    ...
    /// `node` must be null or point to a live `rb_node`.
    #[inline(always)]
    unsafe fn dye_red(node: *mut RbNode) {
        if node.is_null() {
            return;
        }
    ...
    #[inline(always)]
    unsafe fn dye_as(target: *mut RbNode, source: *mut RbNode) {
        unsafe {
            if Self::is_black(source) {
                Self::dye_black(target);
            } else {
                Self::dye_red(target);
            }
        }
...
impl RbRoot {
    ...
    pub(crate) unsafe fn insert_color(root: *mut RbRoot, node: *mut RbNode) -> *mut RbNode {
        unsafe {
            ...
            let mut pos = RbRoot::insert_node(root, node);
            if pos != node {
                return pos;
            }

            RbNode::dye_red(pos);
            let mut parent = RbNode::parent(pos);
            let mut grand_parent = RbNode::parent(parent);
            ...
            while !RbNode::is_black(parent) {
                if parent == (*grand_parent).left {
                    uncle = (*grand_parent).right;
                    if !RbNode::is_black(uncle) {
                        RbNode::dye_black(parent);
                        RbNode::dye_black(uncle);
                        RbNode::dye_red(grand_parent);
                        pos = grand_parent;
                    } else if pos == (*parent).right {
                        RbRoot::rotate_left(root, parent);
                        RbRoot::rotate_right(root, grand_parent);
                        RbNode::dye_black(parent);
                        RbNode::dye_black(grand_parent);
                    } else {
                        RbRoot::rotate_right(root, grand_parent);
                        RbNode::dye_black(pos);
                        RbNode::dye_black(grand_parent);
                        pos = parent;
                    }
                } else {
                    ...
                    if !RbNode::is_black(uncle) {
                        ...
                        RbRoot::rotate_right(root, parent);
                        RbRoot::rotate_left(root, grand_parent);
                        RbNode::dye_black(parent);
                        RbNode::dye_black(grand_parent);
                    } else {
                        RbRoot::rotate_left(root, grand_parent);
                ...
                parent = RbNode::parent(pos);
                grand_parent = RbNode::parent(parent);
            }

            if RbNode::is_top(pos) {
                RbNode::dye_black(pos);
            }
    ...
    unsafe fn delete_color_fixup(root: *mut RbRoot, mut node: *mut RbNode) {
        unsafe {
            let mut brother: *mut RbNode;
            let mut parent = RbNode::parent(node);
            while node != (*root).node && RbNode::is_black(node) {
                if node == RbNode::left(parent) {
                    brother = RbNode::right(parent);
                    if !RbNode::is_black(brother) {
                        RbNode::dye_red(parent);
                        RbNode::dye_black(brother);
                        RbRoot::rotate_left(root, parent);
                        parent = RbNode::parent(node);
                        brother = RbNode::right(parent);
                    }
                    if RbNode::is_black(RbNode::left(brother)) && RbNode::is_black(RbNode::right(brother)) {
                        RbNode::dye_red(brother);
                        node = parent;
                        parent = RbNode::parent(node);
                    } else {
                        if RbNode::is_black(RbNode::right(brother)) {
                            RbNode::dye_black(RbNode::left(brother));
                            RbNode::dye_red(brother);
                            RbRoot::rotate_right(root, brother);
                            parent = RbNode::parent(node);
                            brother = RbNode::right(parent);
                        }

                        RbNode::dye_as(brother, parent);
                        RbNode::dye_black(parent);
                        RbNode::dye_black(RbNode::right(brother));
                        RbRoot::rotate_left(root, parent);
                        node = (*root).node;
                        parent = ptr::null_mut();
                    }
                } else {
                    brother = RbNode::left(parent);
                    if !RbNode::is_black(brother) {
                        RbNode::dye_red(parent);
                        RbNode::dye_black(brother);
                        RbRoot::rotate_right(root, parent);
                    ...
                    if RbNode::is_black(RbNode::left(brother)) && RbNode::is_black(RbNode::right(brother)) {
                        ...
                        if RbNode::is_black(RbNode::left(brother)) {
                            RbNode::dye_black(RbNode::right(brother));
                            RbNode::dye_red(brother);
                            RbRoot::rotate_left(root, brother);
                            parent = RbNode::parent(node);
                            brother = RbNode::left(parent);
                        }
    ...
    unsafe fn do_delete_node_color(
        root: *mut RbRoot,
        _parent: *mut RbNode,
        link: *mut *mut RbNode,
    ) {
        unsafe {
            ...
            if !replacement.is_null() {
                RbRoot::transplant(root, replacement, target);
                ...
                RbNode::set_parent(target, target);
                if RbNode::is_black(target) {
                    RbRoot::delete_color_fixup(root, replacement);
                }
            } else if RbNode::is_top(target) {
                (*root).node = ptr::null_mut();
            } else {
                if RbNode::is_black(target) {
                    RbRoot::delete_color_fixup(root, target);
                }
                if !RbNode::is_top(target) {
                    let target_link = RbRoot::node_link(root, target, ptr::null_mut());
            ...
            if target != delete_node {
                let link = RbRoot::node_link(root, delete_node, ptr::null_mut());
                RbNode::replace(link, target, delete_node);
            }
        }
    }
    ...
    /// `root` must point to a live `rb_root`; `node` must be null or point to
    /// a live `rb_node`.
    pub(crate) unsafe fn delete_node_color(root: *mut RbRoot, node: *mut RbNode) -> *mut RbNode {
        unsafe {
            let mut parent = ptr::null_mut();
            let link = RbRoot::node_link(root, node, ptr::addr_of_mut!(parent));
            if link.is_null() || (*link).is_null() {
                return ptr::null_mut();
            }
            let target = *link;
            RbRoot::do_delete_node_color(root, parent, link);
            target
        }
    }
    ...
    pub(crate) unsafe fn delete_key_color(root: *mut RbRoot, key: u64) -> *mut RbNode {
        unsafe {
            if !RbRoot::is_initialized(root) {
                return ptr::null_mut();
            }
            let mut parent = ptr::null_mut();
            let link = RbRoot::find_key_link(root, ptr::addr_of_mut!(parent), key);
            if link.is_null() || (*link).is_null() {
                return ptr::null_mut();
            }
            let target = *link;
