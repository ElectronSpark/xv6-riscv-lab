...
// Special comparison for "round down" search - finds first node with start_addr
// >= target When keys are equal, return 1 to continue searching left (find
// minimum)
static int ksym_keys_cmp_rdown(uint64 a, uint64 b) {
    ...
    if (a == 0)
        return 0; // Dummy node comparison
    return 1;
}
        ...
        if (*p == '\n' || *p == '\0') {
            ...
            if (line_len == 0) {
                ...
                if (next && *next == ' ') {
                    ...
                    // Store the entry
                    if (__ksymbol_count <
                        (int)(KERNEL_SYMBOLS_IDX_SIZE / sizeof(__ksymbols_t))) {
...
static __ksymbols_t *bt_search_sym(uint64 addr) {
...
int bt_search(uint64 addr, char *buf, size_t buflen, void **return_addr) {
...
static void bt_get_location_sym(__ksymbols_t *sym, char *filebuf,
                                size_t filebuflen, uint32 *line) {
    if (sym == NULL) {
        filebuf[0] = '\0';
        *line = 0;
        return;
...
static int bt_get_offset_sym(__ksymbols_t *sym, uint64 addr) {
    if (sym == NULL) {
        return 0;
    }
...
#define BT_IS_TOP_FRAME(__fp)                                                  \
    (!(uint64)(__fp) || (uint64)(__fp) == PGROUNDDOWN((uint64)(__fp)))
...
// @param kstack_order: order of kernel stack size (size = 1 << (PAGE_SHIFT +
// order))
void print_thread_backtrace(struct context *ctx, uint64 kstack,
    ...
    // First, print the return address from context (where the thread will
    // resume)
    char symbuf[64] = {0};
    char filebuf[128] = {0};
    uint32 line = 0;
    ...
    uint64 last_fp = fp;
    uint64 last_return_addr =
        ctx->ra; // Track last return address to detect loops
    int repeat_count = 0;
