...
uint64 sys_shmat(void)
{
    ...
    /* Map each page (no spinlock held — vm_mmap_region may sleep).
     * After each successful mapping, take an extra page reference so the
     * segment retains ownership of the physical page even after VMA
     * teardown (shmdt / process exit) calls page_ref_dec. */
    for (int i = 0; i < npages; i++) {
        int ret = vm_mmap_region(current->vm,
                                 ...
                                 pages[i]);
        if (ret < 0) {
            /* Unmap already-mapped pages.  vm_munmap_region properly
             * tears down VMA + page_ref_dec per page; the extra
             * page_ref_inc we did below keeps the segment's pages alive. */
            for (int j = 0; j < i; j++)
                vm_munmap_region(current->vm,
                                 shmaddr + (uint64)j * PAGE_SIZE,
                                 PAGE_SIZE);
            /* Undo the pre-increment */
            irq = spin_lock_irqsave(&shm_ids.lock);
            seg->ds.shm_nattch--;
...
uint64 sys_shmdt(void)
{
    ...
    /* Properly unmap each page's VMA.  vm_munmap_region tears down the
     * VMA and calls page_ref_dec; the segment's own reference (from
     * kalloc) keeps the physical page alive until IPC_RMID. */
