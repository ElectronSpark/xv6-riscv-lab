...
static void expect_integrity(const rwsem_t *lock, const char *label) {
    ...
    if (lock->readers > 0 && lock->holder_pid != -1) {
        fail_msg("%s: reader/writer overlap (readers=%d)", label,
                 lock->readers);
    }
...
int main(void) {
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test_setup(test_rwsem_read_acquire_release_integrity,
                               test_setup),
        cmocka_unit_test_setup(test_rwsem_write_acquire_release_integrity,
                               test_setup),
        cmocka_unit_test_setup(test_rwsem_release_wakes_writer_integrity,
                               test_setup),
        cmocka_unit_test_setup(test_rwsem_release_wakes_readers_integrity,
                               test_setup),
    };
