...
static void test_init(void **state) {
    ...
    assert_true((uint64)earalloc_params.current <
                (uint64)(test_memory + TEST_MEMORY_SIZE));
    assert_int_equal(
        (uint64)earalloc_params.current & (EARLYALLOC_SMALLEST_CHUNK - 1), 0);
...
static void test_small_alloc_basic(void **state) {
    ...
    assert_true((uint64)ptr1 + 64 <= (uint64)ptr2 ||
                (uint64)ptr2 + 128 <= (uint64)ptr1);
    assert_true((uint64)ptr1 + 64 <= (uint64)ptr3 ||
                (uint64)ptr3 + 32 <= (uint64)ptr1);
    assert_true((uint64)ptr2 + 128 <= (uint64)ptr3 ||
                (uint64)ptr3 + 32 <= (uint64)ptr2);
}
...
static void test_chunk_splitting(void **state) {
    ...
    // Should not overlap
    assert_true((uint64)ptr2 + 64 <= (uint64)ptr3 ||
                (uint64)ptr3 + 64 <= (uint64)ptr2);
}
...
static void test_multiple_small_from_freelist(void **state) {
    ...
    __free_region_to_chunks((uint64)old_current,
                            (uint64)earalloc_params.current);
...
static void test_chunk_magic(void **state) {
    ...
    for (int i = 0; i < EARLYALLOC_ORDERS; i++) {
        if (!LIST_IS_EMPTY(&earalloc_params.free_lists[i])) {
            list_node_t *entry =
                LIST_FIRST_ENTRY(&earalloc_params.free_lists[i]);
...
int main(void) {
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test_setup_teardown(test_small_alloc_basic, setup_allocator,
                                        teardown_allocator),
        cmocka_unit_test_setup_teardown(test_chunk_splitting, setup_allocator,
                                        teardown_allocator),
        cmocka_unit_test_setup_teardown(test_large_alloc_alignment,
                                        setup_allocator, teardown_allocator),
        cmocka_unit_test_setup_teardown(test_small_alloc_ignores_user_alignment,
                                        setup_allocator, teardown_allocator),
        cmocka_unit_test_setup_teardown(test_alignment_gap_recycling,
                                        setup_allocator, teardown_allocator),
        cmocka_unit_test_setup_teardown(test_end_ptr_tracking, setup_allocator,
                                        teardown_allocator),
        cmocka_unit_test_setup_teardown(test_multiple_small_from_freelist,
                                        setup_allocator, teardown_allocator),
        cmocka_unit_test_setup_teardown(test_size_rounding, setup_allocator,
                                        teardown_allocator),
        cmocka_unit_test_setup_teardown(test_zero_size, setup_allocator,
                                        teardown_allocator),
        cmocka_unit_test_setup_teardown(test_chunk_magic, setup_allocator,
                                        teardown_allocator),
        cmocka_unit_test_setup_teardown(test_stress_many_allocations,
                                        setup_allocator, teardown_allocator),
        cmocka_unit_test_setup_teardown(test_chunk_alignment_verification,
                                        setup_allocator, teardown_allocator),
    };
